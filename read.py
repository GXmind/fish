...
# 编制单位：战略发展部  |  编制日期：2025年4月
 
# ▸ 一、项目背景与目标
 
# 随着市场竞争日趋激烈，公司决策层在2025年第一季度战略会议上提出，需针对核心业务板块开展系统性的可行性研究，以明确下一阶段的资源配置方向与投资优先级。本报告旨在通过定量与定性相结合的方法，对拟实施项目的技术可行性、经济合理性及实施风险进行全面评估。
 
# 经前期市场调研数据显示，目标细分市场年均增速达17.3%，远超行业平均水平，具备较好的市场基础与发展潜力，具体分析详见第三章。
 
# ▸ 二、市场分析
 
# 当前宏观政策环境整体利好，相关产业政策的持续推进为项目落地提供了良好的外部条件。通过对近三年行业数据的纵向比较，市场需求端呈现出明显的结构性转变，高附加值产品占比逐步提升。
 
# 竞争格局分析表明，现有主要竞争者在技术壁垒与成本控制方面均存在明显短板，本项目具备差异化切入的战略窗口期。
 
# ▸ 三、财务预测
 
# 根据保守情景模型，项目预计于第18个月实现盈亏平衡，IRR达23.6%，NPV（折现率10%）为正值，投资回收期约2.4年。详细财务模型及敏感性分析参见附件二。
 
# ▸ 四、风险评估与应对
 
# 主要风险包括市场需求不及预期、核心技术迭代加速及供应链稳定性等。建议建立动态监控机制，每季度对关键假设进行复盘修正，确保项目执行的灵活性与韧性。
 
# ▸ 五、结论与建议
 
# 综合以上分析，本项目在经济性、技术可行性及战略契合度方面均达到立项标准，建议提交董事会审议，并在本季度内完成项目筹备委员会组建工作。
# """   
...
#         # 标题栏
#         hdr = tk.Frame(win, bg='#2b579a', height=30)
#         hdr.pack(fill='x'); hdr.pack_propagate(False)
#         tk.Label(hdr, text='  📝  编辑 Word 伪装文档内容',
#                  font=('', 9, 'bold'), bg='#2b579a', fg='white').pack(side='left', padx=6, pady=5)
#         tk.Button(hdr, text='×', relief='flat', font=('', 11), bg='#2b579a', fg='white',
#                   command=win.destroy).pack(side='right', padx=6)
 
#         tk.Label(win, text='以下内容会显示在老板键伪装的 Word 文档中（Ctrl+D 触发）：',
#                  font=('', 9), bg=bg, fg=fg).pack(anchor='w', padx=12, pady=(10, 4))
 
#         # 文本编辑区
#         ta = tk.Text(win, font=('Microsoft YaHei', 10) if sys.platform == 'win32' else ('', 11),
#                      bg='white', fg='#1a1a1a',
#                      relief='flat', padx=12, pady=10, wrap='word',
#                      borderwidth=1, highlightthickness=1,
#                      highlightbackground='#c8c8c8', highlightcolor='#2b579a')
...
#         else:
#             ta.insert('1.0', _DEFAULT_BOSS_TEXT)
 
#         # 底部按钮
#         bf = tk.Frame(win, bg=bar); bf.pack(fill='x', padx=12, pady=8)
#         tk.Label(bf, text='提示：标题行用"■ "开头，小标题用"▸ "开头',
#                  font=('', 8), bg=bar, fg='#888').pack(side='left')
...
#                 f.write(content)
#             # 如果老板模式已开启，立即刷新文档内容
#             if self._is_boss_mode and hasattr(self, '_boss_text_widget'):
#                 self._refresh_boss_doc(content)
#             self._toast('📝 伪装文档已保存')
#             win.destroy()
 
#         tk.Button(bf, text='保存', font=('', 9, 'bold'),
#                   bg='#2b579a', fg='white', relief='flat', padx=16, pady=4,
#                   command=save_and_close).pack(side='right', padx=4)
#         tk.Button(bf, text='取消', font=('', 9),
#                   bg=bar, fg=fg, relief='flat', padx=12, pady=4,
#                   command=win.destroy).pack(side='right')
 
#         win.bind('<Control-Return>', lambda e: save_and_close())
 
#     # ── 内部辅助：读取文档内容 ────────────────────────────────────────────────
#     def _get_boss_content(self):
#         if os.path.exists(BOSS_FILE):
#             try:
#                 with open(BOSS_FILE, encoding='utf-8') as f:
#                     return f.read()
#             except:
#                 pass
#         return _DEFAULT_BOSS_TEXT
 
#     def _refresh_boss_doc(self, content=None):
#         """把内容渲染进 A4 纸 Text widget（支持简单标记语法）"""
#         if not hasattr(self, '_boss_text_widget'):
#             return
#         if content is None:
#             content = self._get_boss_content()
 
#         tw = self._boss_text_widget
...
#         """构建高仿 Word 界面（仅创建一次，之后复用）"""
#         WORD_BLUE  = '#2b579a'
#         RIBBON_BG  = '#f3f2f1'
#         DOC_GRAY   = '#b0b0b0'
#         BORDER_CLR = '#c8c8c8'
 
#         self.boss_frame = tk.Frame(self.root, bg=DOC_GRAY)
 
#         # ── 1. 标题栏 ────────────────────────────────────────────────────────
#         title_bar = tk.Frame(self.boss_frame, bg=WORD_BLUE, height=28)
#         title_bar.pack(fill='x'); title_bar.pack_propagate(False)
 
#         tk.Label(title_bar, text='💾  ✔  ↶  ↷',
#                  font=('', 9), bg=WORD_BLUE, fg='#c7d8f0').pack(side='left', padx=8)
#         tk.Label(title_bar, text='项目可行性研究报告.docx - Word',
#                  font=('', 9, 'bold'), bg=WORD_BLUE, fg='white').pack(side='left', expand=True)
 
#         # 窗口控制按钮
#         for sym, hov in [('─', WORD_BLUE), ('⬜', WORD_BLUE), ('✕', '#c42b1c')]:
#             b = tk.Label(title_bar, text=sym, font=('', 10), bg=WORD_BLUE, fg='white',
#                          width=4, cursor='hand2')
#             b.pack(side='right')
#             b.bind('<Enter>', lambda e, w=b, c=hov: w.config(bg=c))
#             b.bind('<Leave>', lambda e, w=b: w.config(bg=WORD_BLUE))
 
#         # ── 2. 菜单栏 ────────────────────────────────────────────────────────
#         menu_bar = tk.Frame(self.boss_frame, bg=WORD_BLUE, height=24)
#         menu_bar.pack(fill='x'); menu_bar.pack_propagate(False)
 
#         menus = ['文件', '开始', '插入', '设计', '布局', '引用', '邮件', '审阅', '视图', '帮助']
#         for i, m in enumerate(menus):
#             is_active = (i == 1)  # "开始" 高亮
#             bg_c = 'white'        if is_active else WORD_BLUE
#             fg_c = WORD_BLUE      if is_active else '#d0dff5'
#             lbl = tk.Label(menu_bar, text=m, font=('', 9), bg=bg_c, fg=fg_c,
#                            padx=10, pady=2, cursor='hand2')
#             lbl.pack(side='left')
#             if not is_active:
#                 lbl.bind('<Enter>', lambda e, w=lbl: w.config(bg='#3f6bb5', fg='white'))
#                 lbl.bind('<Leave>', lambda e, w=lbl: w.config(bg=WORD_BLUE, fg='#d0dff5'))
 
#         # ── 3. Ribbon ─────────────────────────────────────────────────────────
#         ribbon = tk.Frame(self.boss_frame, bg='white',
#                           highlightbackground=BORDER_CLR, highlightthickness=1)
#         ribbon.pack(fill='x')
 
#         ribbon_inner = tk.Frame(ribbon, bg='white')
#         ribbon_inner.pack(fill='x', padx=4, pady=0)
 
#         def _sep():
#             tk.Frame(ribbon_inner, width=1, bg=BORDER_CLR).pack(
#                 side='left', fill='y', pady=4, padx=2)
 
#         def _rg_label(parent, text):
#             tk.Label(parent, text=text, font=('', 8), fg='#666', bg='white').pack(side='bottom', pady=1)
 
#         def _rb(parent, icon, label, large=False):
#             f = tk.Frame(parent, bg='white', cursor='hand2')
#             f.pack(side='left', padx=1, pady=2)
#             sz = 18 if large else 13
#             tk.Label(f, text=icon, font=('', sz), bg='white').pack()
#             tk.Label(f, text=label, font=('', 8), fg='#333', bg='white').pack()
#             f.bind('<Enter>', lambda e: f.config(bg='#e9ecf3'))
#             f.bind('<Leave>', lambda e: f.config(bg='white'))
#             for w in f.winfo_children():
#                 w.bind('<Enter>', lambda e, fw=f: fw.config(bg='#e9ecf3'))
#                 w.bind('<Leave>', lambda e, fw=f: fw.config(bg='white'))
 
#         # 剪贴板组
#         g1 = tk.Frame(ribbon_inner, bg='white'); g1.pack(side='left', fill='y', padx=2)
#         g1c = tk.Frame(g1, bg='white'); g1c.pack(fill='both', expand=True)
#         _rb(g1c, '📋', '粘贴', large=True)
#         g1r = tk.Frame(g1c, bg='white'); g1r.pack(side='left')
#         for ic, lb in [('✂', '剪切'), ('⧉', '复制'), ('🖊', '格式刷')]:
#             f2 = tk.Frame(g1r, bg='white', cursor='hand2'); f2.pack(anchor='w')
#             tk.Label(f2, text=ic, font=('', 11), bg='white').pack(side='left', padx=2)
#             tk.Label(f2, text=lb, font=('', 8), bg='white', fg='#333').pack(side='left')
#             f2.bind('<Enter>', lambda e, fw=f2: fw.config(bg='#e9ecf3'))
#             f2.bind('<Leave>', lambda e, fw=f2: fw.config(bg='white'))
#         _rg_label(g1, '剪贴板')
#         _sep()
 
#         # 字体组
#         g2 = tk.Frame(ribbon_inner, bg='white'); g2.pack(side='left', fill='y', padx=2)
#         font_nm = 'Microsoft YaHei' if sys.platform == 'win32' else 'Arial'
#         row1 = tk.Frame(g2, bg='white'); row1.pack(anchor='w', pady=1)
#         tk.Label(row1, text='等线 (中文正文)  ▾', font=('', 9), bg='white',
#                  relief='groove', bd=1, padx=4, width=16).pack(side='left')
#         tk.Label(row1, text='11 ▾', font=('', 9), bg='white',
#                  relief='groove', bd=1, padx=4, width=4).pack(side='left', padx=2)
#         tk.Label(row1, text='A↑ A↓', font=('', 9), bg='white').pack(side='left', padx=2)
#         row2 = tk.Frame(g2, bg='white'); row2.pack(anchor='w', pady=1)
#         for txt, kw in [('B', {'font':('',11,'bold')}), ('I', {'font':('',11,'italic')}),
#                         ('U̲', {'font':('',11)}), ('ab̶', {'font':('',9)}),
#                         ('A', {'font':('',11)}), ('A', {'font':('',11), 'fg':'#c00'})]:
#             tk.Label(row2, text=txt, bg='white', relief='flat', padx=3,
#                      cursor='hand2', **kw).pack(side='left')
#         _rg_label(g2, '字体')
#         _sep()
 
#         # 段落组
#         g3 = tk.Frame(ribbon_inner, bg='white'); g3.pack(side='left', fill='y', padx=2)
#         r3a = tk.Frame(g3, bg='white'); r3a.pack(anchor='w', pady=1)
#         for t in ['≡▾', '½≡▾', '|', '☰', '≡', '☲', '☳']:
#             tk.Label(r3a, text=t, font=('', 10), bg='white', padx=2).pack(side='left')
#         r3b = tk.Frame(g3, bg='white'); r3b.pack(anchor='w', pady=1)
#         for t in ['⬛', '↕', '↵', '¶']:
#             tk.Label(r3b, text=t, font=('', 10), bg='white', padx=2).pack(side='left')
#         _rg_label(g3, '段落')
#         _sep()
 
#         # 样式组
#         g4 = tk.Frame(ribbon_inner, bg='white'); g4.pack(side='left', fill='y', padx=2)
#         styles_f = tk.Frame(g4, bg='white'); styles_f.pack(fill='both', expand=True, pady=2)
#         style_defs = [
#             ('正文',  '#000', '#f0f4fb', True),
#             ('标题 1', WORD_BLUE, 'white', False),
#             ('标题 2', WORD_BLUE, 'white', False),
#         ]
#         for s_lbl, s_fg, s_bg, s_active in style_defs:
#             sf = tk.Frame(styles_f, bg=s_bg,
#                           highlightbackground=BORDER_CLR if s_active else 'white',
#                           highlightthickness=1, cursor='hand2', width=50)
#             sf.pack(side='left', padx=1)
#             tk.Label(sf, text='AaBbCc', font=('Times New Roman', 9),
#                      fg=s_fg, bg=s_bg, padx=4, pady=1).pack()
#             tk.Label(sf, text=s_lbl, font=('', 8), fg=s_fg, bg=s_bg, padx=4).pack()
#         _rg_label(g4, '样式')
#         _sep()
 
#         # 编辑组
#         g5 = tk.Frame(ribbon_inner, bg='white'); g5.pack(side='left', fill='y', padx=2)
#         for ic, lb in [('🔍', '查找 ▾'), ('⇄', '替换'), ('↖', '选择 ▾')]:
#             f5 = tk.Frame(g5, bg='white', cursor='hand2'); f5.pack(anchor='w', pady=1)
#             tk.Label(f5, text=ic, font=('', 11), bg='white').pack(side='left', padx=2)
#             tk.Label(f5, text=lb, font=('', 9), bg='white', fg='#333').pack(side='left')
#         _rg_label(g5, '编辑')
 
#         # Ribbon 底部提示（Ctrl+D 退出）
#         hint = tk.Frame(self.boss_frame, bg=RIBBON_BG, height=14)
#         hint.pack(fill='x'); hint.pack_propagate(False)
#         tk.Label(hint, text='按 Ctrl+D 恢复阅读', font=('', 8), bg=RIBBON_BG, fg='#999').pack(side='right', padx=8)
 
#         # ── 4. 标尺 ────────────────────────────────────────────────────────────
#         ruler = tk.Canvas(self.boss_frame, height=16, bg='#f0eeeb',
#                           highlightthickness=0)
#         ruler.pack(fill='x')
#         # 绘制简单标尺刻度
#         self.boss_frame.after(50, lambda: self._draw_ruler(ruler))
 
#         # ── 5. 文档主区域（A4 纸）─────────────────────────────────────────────
#         doc_outer = tk.Frame(self.boss_frame, bg=DOC_GRAY)
#         doc_outer.pack(fill='both', expand=True)
 
#         # 右侧滚动条
#         doc_vsb = tk.Scrollbar(doc_outer, orient='vertical', width=14)
#         doc_vsb.pack(side='right', fill='y')
 
#         doc_canvas = tk.Canvas(doc_outer, bg=DOC_GRAY,
#                                highlightthickness=0,
#                                yscrollcommand=doc_vsb.set)
#         doc_canvas.pack(side='left', fill='both', expand=True)
#         doc_vsb.config(command=doc_canvas.yview)
 
#         # A4 纸 Frame（白色，固定宽度居中）
#         a4_outer = tk.Frame(doc_canvas, bg=DOC_GRAY)
#         doc_canvas.create_window((0, 0), window=a4_outer, anchor='nw')
 
#         # 纸张容器（阴影感用边框模拟）
#         a4_shadow = tk.Frame(a4_outer, bg='#888', padx=2, pady=2)
#         a4_shadow.pack(pady=20)
 
#         a4 = tk.Frame(a4_shadow, bg='white', width=680)
#         a4.pack()
#         a4.pack_propagate(False)
 
#         # A4 内容 Text widget
#         doc_font = ('Times New Roman', 11) if sys.platform != 'linux' else ('', 11)
#         tw = tk.Text(a4, font=doc_font,
#                      bg='white', fg='#1a1a1a',
#                      relief='flat', padx=80, pady=72,
...
#         # 文字样式标签
#         h1_font = ('Times New Roman', 16, 'bold') if sys.platform != 'linux' else ('', 16, 'bold')
#         h2_font = ('Times New Roman', 13, 'bold') if sys.platform != 'linux' else ('', 13, 'bold')
#         tw.tag_config('h1',     font=h1_font, foreground=WORD_BLUE,
#                       spacing1=14, spacing3=6)
#         tw.tag_config('h2',     font=h2_font, foreground=WORD_BLUE,
#                       spacing1=10, spacing3=4)
#         tw.tag_config('normal', font=doc_font, foreground='#1a1a1a',
#                       lmargin1=0, lmargin2=0, spacing1=2, spacing3=2)
 
#         self._boss_text_widget = tw
#         self._boss_doc_canvas  = doc_canvas
#         self._boss_a4_outer    = a4_outer
 
#         # 滚轮绑定
#         def _boss_scroll(e):
#             delta = -1 if (e.delta > 0 or e.num == 4) else 1
#             doc_canvas.yview_scroll(delta * 3, 'units')
#         doc_canvas.bind('<MouseWheel>', _boss_scroll)
#         doc_canvas.bind('<Button-4>',   _boss_scroll)
#         doc_canvas.bind('<Button-5>',   _boss_scroll)
#         tw.bind('<MouseWheel>', _boss_scroll)
 
#         # Canvas 自适应大小
#         def _on_configure(e):
#             doc_canvas.configure(scrollregion=doc_canvas.bbox('all'))
#             # 水平居中 A4 纸
#             cw = doc_canvas.winfo_width()
#             aw = a4_outer.winfo_reqwidth()
#             x  = max(0, (cw - aw) // 2)
#             doc_canvas.coords(doc_canvas.find_all()[0], x, 0)
#         a4_outer.bind('<Configure>', _on_configure)
 
#         # ── 6. 状态栏 ────────────────────────────────────────────────────────
#         status = tk.Frame(self.boss_frame, bg=WORD_BLUE, height=22)
#         status.pack(fill='x', side='bottom'); status.pack_propagate(False)
 
#         tk.Label(status, text='第 1 页，共 5 页  |  2,847 个字  |  中文(中国)',
#                  font=('', 8), bg=WORD_BLUE, fg='#c7d8f0').pack(side='left', padx=10)
#         # 右侧视图按钮 + 缩放
#         right_f = tk.Frame(status, bg=WORD_BLUE); right_f.pack(side='right', padx=8)
#         for sym, active in [('📄', True), ('🌐', False), ('📖', False)]:
#             c = 'white' if active else '#c7d8f0'
#             tk.Label(right_f, text=sym, font=('', 9), bg=WORD_BLUE, fg=c,
#                      cursor='hand2').pack(side='left', padx=2)
#         tk.Label(right_f, text='  ─────  100%',
#                  font=('', 8), bg=WORD_BLUE, fg='#c7d8f0').pack(side='left', padx=4)
...
#             H = 16
#             canvas.delete('all')
#             canvas.create_rectangle(0, 0, W, H, fill='#f0eeeb', outline='')
#             # 灰色文档区（页边距外）
#             margin_px = max(0, (W - 520) // 2)
#             canvas.create_rectangle(0, 0, margin_px, H, fill='#d0cec9', outline='')
#             canvas.create_rectangle(W - margin_px, 0, W, H, fill='#d0cec9', outline='')
#             # 刻度线
#             for i in range(0, 26):
#                 x = margin_px + i * 20
#                 if x > W - margin_px: break
#                 h_tick = 8 if i % 5 == 0 else 4
#                 canvas.create_line(x, H - h_tick, x, H, fill='#888', width=1)
#                 if i % 5 == 0 and i > 0:
#                     canvas.create_text(x, 3, text=str(i), font=('', 7), fill='#666', anchor='n')
#         except:
#             pass
 
#     def toggle_boss_mode(self, event=None):
#         """Ctrl+D：在阅读模式 ↔ Word 伪装模式之间切换"""
#         if not hasattr(self, 'boss_frame'):
#             self._build_boss_mode_ui()
...
#             # 隐藏阅读界面
#             for w in (self.topbar, self.setbar, self.sep,
#                       self.txt_frame, self.bot_area):
#                 w.pack_forget()
...
#             # 刷新文档内容
#             self._refresh_boss_doc()
 
#             # 根据屏幕大小调整窗口（模拟 Word 默认大小）
...
#             self.root.title('项目可行性研究报告.docx - Word')
...
#             # 重绘标尺（窗口宽度变了）
#             self.root.after(100, lambda: self._draw_ruler(
#                 [w for w in self.boss_frame.winfo_children()
#                  if isinstance(w, tk.Canvas)][0]
#                 if any(isinstance(w, tk.Canvas)
#                        for w in self.boss_frame.winfo_children()) else None
#             ))
...
#             self.root.attributes('-alpha', self._real_alpha)
 
#             # 恢复阅读界面
#             self._apply_theme()   # 先刷新颜色
...
#     """
#     根据选择的模型类型读取对应的 API Key
#     GLM 使用 api_key.txt
#     MiniMax 使用 api_key_minimax.txt
#     """
#     filename = 'api_key_minimax.txt' if model_type == 'MiniMax' else 'api_key.txt'
#     env_var = 'MINIMAX_API_KEY' if model_type == 'MiniMax' else 'ZHIPU_API_KEY'
...
#         # ── 新增：AI模型切换 ─────────────────────────
#         r5 = row()
#         tk.Label(r5, text='AI模型', font=('',8)).pack(side='left')
#         self.var_ai_model = tk.StringVar(value='GLM (智谱)')
#         cb_ai = ttk.Combobox(r5, textvariable=self.var_ai_model,
#                              values=['GLM (智谱)', 'MiniMax'], width=12, font=('',8), state='readonly')
#         cb_ai.pack(side='left', padx=4)
...
#             flags = 0 if var_case.get() else re.IGNORECASE
#             try:
#                 if var_regex.get(): pat = re.compile(query, flags)
#                 else:               pat = re.compile(re.escape(query), flags)
#             except re.error as e:
#                 lbl_count.config(text=f'正则错误: {e}'); return
...
#             lbl_loc.config(text=f'第{ci+1}章 · 位置 {s}')
#             btn_jump.config(state='normal', command=lambda: _jump(ci, s, e2, hit))
...
#             if msg.startswith('NO_KEY_'):
#                 _model = msg.replace('NO_KEY_', '')
#                 _fname = 'api_key_minimax.txt' if _model == 'MiniMax' else 'api_key.txt'
#                 _show(
#                     f'⚠  未找到 {_model} 的 API Key\n\n'
#                     f'请在程序同目录新建文件：{_fname}\n'
#                     f'将 API Key 粘贴进去保存即可。',
#                     'warn'
#                 )
#             else:
#                 _show(f'⚠  生成失败\n\n{msg}', 'warn')
...
#             _model_choice = self.var_ai_model.get()
#             _show(f'[{_model_choice}] 正在生成【{_len_label}·{_sty_label}】摘要，请稍候…', 'loading')
...
#                 if _model_choice == 'MiniMax':
#                     _key = _get_api_key('MiniMax')
#                     _url = 'https://api.minimax.chat/v1/chat/completions'
#                     _model_name = 'abab6.5s-chat'
#                 else:
#                     _key = _get_api_key('GLM')
#                     _url = 'https://open.bigmodel.cn/api/paas/v4/chat/completions'
#                     _model_name = 'glm-4-flash'

#                 if not _key:
#                     dlg.after(0, lambda: _on_err(f'NO_KEY_{_model_choice}'))
#                     return
...
#                     try:    _hemsg = _ujson.loads(_hbody).get('error',{}).get('message', _hbody)
#                     except: _hemsg = _hbody[:200]
#                     dlg.after(0, lambda c=str(_he.code), m=_hemsg: _on_err(f'HTTP {c}：{m}'))
#                 except Exception as _exc:
#                     dlg.after(0, lambda m=str(_exc): _on_err(m))
...
#         if total_ch > CHUNK_SIZE:
#             range_opts.append("全书 (仅匹配标题不含正文)")
...
#                 _model = msg.replace('NO_KEY_', '')
#                 _fname = 'api_key_minimax.txt' if _model == 'MiniMax' else 'api_key.txt'
#                 _show_detail(f'⚠  未找到 {_model} 的 API Key\n请在程序同目录新建 {_fname}', 'warn')
#             else:
#                 _show_detail(f'⚠  分析失败\n\n{msg}', 'warn')
#             lbl_st2.config(text='')
...
#             _selected_range = v_range.get()
#             _model_choice = self.var_ai_model.get()
#             _show_detail(f'[{_model_choice}] 正在分析 [{_selected_range}]，请稍候…', 'hint')
#             _top_n = v_cnt.get()
...
#                 if not _key:
#                     dlg.after(0, lambda: _on_find_error(f'NO_KEY_{_model_choice}')); return
...
#         r.bind('<Control-g>',    lambda e: self.open_chapter_finder())
#         r.bind('<Control-G>',    lambda e: self.open_chapter_finder())
#         r.bind('<Control-d>',    self.toggle_boss_mode) # ★ 唤醒 Word 老板键 ★
#         r.bind('<Control-D>',    self.toggle_boss_mode)
#         r.bind('<KeyPress-d>',   self._dp)
#         r.bind('<KeyPress-D>',   self._dp)
#         r.bind('<KeyRelease-d>', self._dr)
...
摸鱼阅读器 v9.0  —  四项核心优化版
优化：
  [问题1-方案B] 角色分析：本地关键词索引，全文扫描角色相关段落再送AI，准确度大幅提升
  [问题2-方案A] 文章分析：5维度结构化Prompt（主题/人物弧/伏笔/高潮/金句），深度解析
  [问题3-方案A] 找章节：两阶段检索，本地TF-IDF粗筛→AI精排，准确率大幅提升
  [问题4-方案A] 界面：精致扁平风，圆角卡片，分组图标按钮，章节进度渐变条
...
import tkinter as tk
from tkinter import ttk, filedialog, colorchooser, messagebox, scrolledtext
import os, re, zipfile, sys, json, copy, threading, time, datetime, math
from html.parser import HTMLParser
...
# ══════════════════════════════════════════════════════════════
# ★ 问题3-方案A：本地 TF-IDF 粗筛工具函数 ★
# ══════════════════════════════════════════════════════════════
def _tokenize_zh(text):
    """简单中文分词：按字符ngram + 提取连续汉字词组"""
    # 提取2-4字汉字词组
    tokens = []
...
def tfidf_rank_chapters(query, chapters, top_n=20):
    """
    本地TF-IDF粗筛：
    1. 对query分词，得到查询词列表
    2. 统计每章包含查询词的TF分数
    ...
    if not q_tokens:
        ...
        for i, ch in enumerate(chapters):
            score = sum(ch['body'].count(w) for w in query.split() if w)
    ...
    # 计算IDF：含有该词的章节数
    doc_freq = Counter()
    total_docs = len(chapters)
    for ch in chapters:
        ch_tokens = set(_tokenize_zh(ch['body'] + ch['title']))
        for tok in q_tokens:
            if tok in ch_tokens:
                doc_freq[tok] += 1
    ...
    for i, ch in enumerate(chapters):
        text = ch['title'] + '\n' + ch['body']
        ch_tokens = _tokenize_zh(text)
        ch_total = max(1, len(ch_tokens))
        ch_counter = Counter(ch_tokens)

        score = 0.0
        for tok in q_tokens:
            tf = ch_counter.get(tok, 0) / ch_total
            df = doc_freq.get(tok, 0)
            idf = math.log((total_docs + 1) / (df + 1)) + 1
            # 标题命中加权
            title_bonus = 3.0 if tok in ch['title'] else 1.0
    ...
    scores.sort(key=lambda x: -x[1])
    # 过滤分数为0的
    valid = [(i, s) for i, s in scores if s > 0]
    if not valid:
        # 完全没命中时退化为返回均匀分布的章节
        step = max(1, total_docs // top_n)
        return list(range(0, total_docs, step))[:top_n]
...
# ★ 问题1-方案B：角色关键词索引提取 ★
# ══════════════════════════════════════════════════════════════
def extract_role_context(role_name, chapters, cur_ch_idx, max_chars=8000):
    """
    全文扫描含有角色名的段落，按位置权重合并，最多返回 max_chars 字。
    近处章节权重更高（近因效应），保证质量。
    """
    segments = []  # [(chapter_idx, paragraph, weight)]

    for ch_idx, ch in enumerate(chapters):
        # 距离当前章节越近权重越高
        dist = abs(ch_idx - cur_ch_idx)
        weight = 1.0 / (1.0 + dist * 0.1)

        paragraphs = [p.strip() for p in ch['body'].split('\n') if p.strip()]
        for para in paragraphs:
            if role_name in para:
                # 对话段（含引号）额外加权
                has_dialog = bool(re.search(r'[""「」『』]', para))
                w = weight * (1.5 if has_dialog else 1.0)
                segments.append((ch_idx, para, w))

    if not segments:
        # 降级：取当前章及前后各3章的全文
        start = max(0, cur_ch_idx - 3)
        end   = min(len(chapters), cur_ch_idx + 4)
        fallback = '\n'.join(ch['body'] for ch in chapters[start:end])
    ...
    total = 0
    seen = set()
    for ch_idx, para, w in segments:
        key = para[:30]  # 去重
        if key in seen: continue
        seen.add(key)
        if total + len(para) > max_chars: break
        result_parts.append(f"[第{ch_idx+1}章] {para}")
        total += len(para)
...
NOTES_FILE = os.path.join(DATA_DIR, 'notes.json')
MARKS_FILE = os.path.join(DATA_DIR, 'marks.json')
BOSS_FILE  = os.path.join(DATA_DIR, 'boss_text.txt')
PROFILES_DIR = os.path.join(DATA_DIR, 'profiles')
os.makedirs(PROFILES_DIR, exist_ok=True)
...
class TTS:
    def __init__(self):
        self._engine = None; self._thread = None
        self._stop   = threading.Event(); self._rate = 180; self._avail = False
        self._init()

    def _init(self):
        try:
            import pyttsx3
            self._engine = pyttsx3.init(); self._avail = True
    ...
    def speak(self, text, on_done=None):
        if not self._avail: return
        self.stop(); self._stop.clear(); self._text = text
        def run():
            try:
                import pyttsx3
                eng = pyttsx3.init(); eng.setProperty('rate', self._rate)
                for para in [p.strip() for p in text.split('\n') if p.strip()]:
                    if self._stop.is_set(): break
                    eng.say(para); eng.runAndWait()
                eng.stop()
            except: pass
            if on_done: on_done()
        self._thread = threading.Thread(target=run, daemon=True)
...
class SmoothScrollbar:
    ...
    def _draw(self):
        ...
        cv.delete('all'); PAD=3
        cv.create_rectangle(0,0,W,H,fill=self.c_bg,outline='')
        th=max(24,int((self._hi-self._lo)*(H-2*PAD))); ty=PAD+int(self._lo*(H-2*PAD))
        ty2=min(H-PAD,ty+th); color=self.c_hover if self._hovered else self.c_thumb
        r=W//2-1; x1,x2=1,W-1; y1,y2=ty,ty2
        if y2-y1>=2*r:
            cv.create_rectangle(x1,y1+r,x2,y2-r,fill=color,outline='')
            cv.create_oval(x1,y1,x2,y1+2*r,fill=color,outline='')
            cv.create_oval(x1,y2-2*r,x2,y2,fill=color,outline='')
    ...
    def _drag(self, e):
        if not self._dragging: return
        H=self.cv.winfo_height(); PAD=3; dy=e.y-self._drag_start_y
        delta=dy/max(1,H-2*PAD); span=self._hi-self._lo
        target=max(0.0,min(1.0-span,self._drag_start_lo+delta))
        self._lo=target; self._hi=target+span; self._draw(); self.on_scroll(target)
...
# ══════════════════════════════════════════════════════════════
# ★ 问题4-方案A：精致扁平风 UI 工具函数 ★
# ══════════════════════════════════════════════════════════════
def _make_icon_btn(parent, icon, label, cmd, bg, fg, hover_bg, font_size=9):
    """圆角卡片式图标按钮：图标在上，文字在下"""
    frame = tk.Frame(parent, bg=bg, cursor='hand2', padx=3, pady=2)
    lbl_icon = tk.Label(frame, text=icon, font=('', font_size+1), bg=bg, fg=fg)
    lbl_icon.pack()
    lbl_text = tk.Label(frame, text=label, font=('', 7), bg=bg, fg=fg)
    lbl_text.pack()

    def on_enter(e):
        frame.config(bg=hover_bg)
        for w in frame.winfo_children(): w.config(bg=hover_bg)
    def on_leave(e):
        frame.config(bg=bg)
        for w in frame.winfo_children(): w.config(bg=bg)
    def on_click(e): cmd()
...
def _make_flat_btn(parent, text, cmd, bg, fg, hover_bg=None, bold=False, padx=8, pady=3):
    """简洁扁平按钮"""
    if hover_bg is None: hover_bg = fg + '22' if len(fg)==7 else '#cccccc'
...
def _draw_progress_bar(canvas, pct, bg_color, fill_color, accent_color, width, height=4):
    """渐变进度条（用两段颜色模拟渐变）"""
    canvas.delete('all')
    ...
    # 填充段
    fill_w = max(r*2, int(width * pct / 100))
    if fill_w > 0:
        # 渐变模拟：分两段
        mid = fill_w * 6 // 10
        canvas.create_rectangle(0, 0, fill_w, height, fill=fill_color, outline='')
        if mid > 0:
            canvas.create_rectangle(0, 0, mid, height, fill=accent_color, outline='')
...
class App:
    ...
    THEMES = {
        '暖黄': dict(bg='#fdf6e3',fg='#3c3836',bar='#ebdbb2',sel='#d5c4a1',
                    thumb='#b8a882',thumb_h='#8f7a55',accent='#d79921',card='#f5edd6'),
        '夜间': dict(bg='#1e1e2e',fg='#cdd6f4',bar='#181825',sel='#313244',
                    thumb='#45475a',thumb_h='#6c7086',accent='#89b4fa',card='#24273a'),
        '护眼': dict(bg='#1a2f1a',fg='#a8d5a2',bar='#152315',sel='#2d5a2d',
                    thumb='#3a6b3a',thumb_h='#57c454',accent='#57c454',card='#1e3a1e'),
        '纸张': dict(bg='#f5f0e8',fg='#2c2416',bar='#ede4d0',sel='#c8b89a',
                    thumb='#c8a882',thumb_h='#8b6045',accent='#8b6045',card='#ede4d0'),
        '白底': dict(bg='#ffffff',fg='#1a1a1a',bar='#f5f5f5',sel='#e0e0e0',
                    thumb='#bbbbbb',thumb_h='#888888',accent='#4a90d9',card='#f0f0f0'),
    }
    ...
    def __init__(self):
        ...
        self.shelf   = _jload(SHELF_FILE)
        self.notes   = _jload(NOTES_FILE)
        self.marks   = _jload(MARKS_FILE)
    ...
    def _build_ui(self):
        ...
        # ── 顶部栏（高度提升，分组设计）────────────────
        self.topbar = tk.Frame(r, height=44)
        self.topbar.pack(fill='x')
        self.topbar.pack_propagate(False)

        # 左侧：交通灯 + 标题
        left_f = tk.Frame(self.topbar); left_f.pack(side='left', fill='y', padx=6)
        dots_f = tk.Frame(left_f); dots_f.pack(side='left', padx=2, pady=14)
        self._dot(dots_f, '#ff5f57', self.toggle_minimize, '─')
        self._dot(dots_f, '#ffbd2e', self.toggle_settings, '⚙')
        self._dot(dots_f, '#28ca41', self.open_file,       '＋')

        self.lbl_title = tk.Label(self.topbar, text='摸鱼阅读器 v9',
                                   font=('', 10, 'bold'), anchor='w')
        self.lbl_title.pack(side='left', padx=8, fill='y')

        # 右侧：功能图标组（紧凑排列）
        rbf = tk.Frame(self.topbar); rbf.pack(side='right', padx=6, fill='y')
        # 分隔线
        self._vline = tk.Frame(rbf, width=1); self._vline.pack(side='right', fill='y', pady=8, padx=2)
        ...
        btn_defs = [
            ...
            ('📑', '目录',  self.open_chapters),
            ('🔍', '搜索',  self.open_search),
            ('✨', '摘要',  self.open_summary),
            ('🔮', '找章',  self.open_chapter_finder),
            ('✏',  '笔记',  self.open_notes),
            ('🔖', '书签',  self.open_marks),
            ('💬', '对话',  self.open_role_chat),
        ]
        init_theme = self._cur_theme()
        
        for icon, label, cmd in reversed(btn_defs):
            btn = _make_flat_btn(rbf, f'{icon}' if not label else f'{icon}{label}',
                                  cmd, bg=init_theme['bar'], fg=init_theme['fg'], padx=5, pady=11)
            btn.pack(side='right', padx=1)
            self._icon_btns.append(btn)

        for w in (self.topbar, self.lbl_title, left_f, dots_f):
            w.bind('<ButtonPress-1>', self._drag_start)
            w.bind('<B1-Motion>',     self._drag_move)

        # ── 进度条（章节渐变进度，高度4px）────────────
        self.prog_canvas = tk.Canvas(r, height=3, highlightthickness=0)
        ...
        self.botbar = tk.Frame(self.bot_area, height=36)
        self.botbar.pack(fill='x')
        self.botbar.pack_propagate(False)

        self.btn_prev = tk.Label(self.botbar, text='◀', font=('', 11),
                                  cursor='hand2', padx=8)
        self.btn_prev.pack(side='left', pady=6)
        self.btn_prev.bind('<Button-1>', lambda e: self.prev_chapter())

        self.lbl_prog = tk.Label(self.botbar, text='', font=('', 9))
        self.lbl_prog.pack(side='left', expand=True)

        # 右侧功能按钮组（卡片式）
        action_f = tk.Frame(self.botbar); action_f.pack(side='right', pady=4, padx=6)
        self.btn_tts  = tk.Label(action_f, text='🔊', font=('',11), cursor='hand2', padx=5)
        self.btn_note = tk.Label(action_f, text='✏',  font=('',11), cursor='hand2', padx=5)
        self.btn_mark = tk.Label(action_f, text='🔖', font=('',11), cursor='hand2', padx=5)
        self.btn_tts.pack(side='left')
        self.btn_note.pack(side='left')
        self.btn_mark.pack(side='left')
        self.btn_tts.bind('<Button-1>',  lambda e: self.toggle_tts())
        self.btn_note.bind('<Button-1>', lambda e: self.add_note())
        self.btn_mark.bind('<Button-1>', lambda e: self.add_mark())

        self.btn_next = tk.Label(self.botbar, text='▶', font=('', 11),
                                  cursor='hand2', padx=8)
        self.btn_next.pack(side='right', pady=6)
        self.btn_next.bind('<Button-1>', lambda e: self.next_chapter())

    def _dot(self, p, color, cmd, sym=''):
        f = tk.Frame(p, bg=color, width=13, height=13, cursor='hand2')
        f.pack(side='left', padx=2)
        f.pack_propagate(False)
        f.bind('<Button-1>', lambda e: cmd())
        f.bind('<Enter>', lambda e, w=f: w.config(relief='sunken'))
        f.bind('<Leave>', lambda e, w=f: w.config(relief='flat'))
    ...
    def _build_setbar(self):
        def row(pady=3):
            f=tk.Frame(self.setbar); f.pack(fill='x', padx=10, pady=pady); return f

        # 第一行：字号 + 行距
        r1 = row()
        tk.Label(r1,text='Aa 字号',font=('',8,'bold')).pack(side='left')
        self.sl_font = tk.Scale(r1,from_=10,to=32,orient='horizontal',length=75,
                                showvalue=True,font=('',7),command=self._on_font_size)
        self.sl_font.set(self.font_size); self.sl_font.pack(side='left',padx=4)
        tk.Frame(r1,width=1).pack(side='left',padx=4)
        tk.Label(r1,text='≡ 行距',font=('',8,'bold')).pack(side='left')
        self.sl_spacing = tk.Scale(r1,from_=0,to=24,orient='horizontal',length=75,
                                   showvalue=True,font=('',7),command=self._on_spacing)
        self.sl_spacing.set(self.line_spacing); self.sl_spacing.pack(side='left',padx=4)

        # 第二行：透明度 + TTS语速
        r2 = row()
        tk.Label(r2,text='◑ 透明',font=('',8,'bold')).pack(side='left')
        self.sl_alpha = tk.Scale(r2,from_=20,to=100,orient='horizontal',length=75,
                                 showvalue=True,font=('',7),
                                 command=lambda v: self.root.attributes('-alpha',int(v)/100))
        self.sl_alpha.set(100); self.sl_alpha.pack(side='left',padx=4)
        tk.Frame(r2,width=1).pack(side='left',padx=4)
        tk.Label(r2,text='🔊 语速',font=('',8,'bold')).pack(side='left')
        self.sl_tts_rate = tk.Scale(r2,from_=80,to=300,orient='horizontal',length=75,
                                    showvalue=True,font=('',7),
                                    command=lambda v: self.tts.set_rate(int(v)))
        self.sl_tts_rate.set(180); self.sl_tts_rate.pack(side='left',padx=4)

        # 第三行：颜色控制
        r3 = row()
        tk.Label(r3,text='🎨 颜色',font=('',8,'bold')).pack(side='left')
        self.btn_fg_color = tk.Label(r3,text='字色',relief='groove',
                                     font=('',8),cursor='hand2',padx=6,pady=2)
        self.btn_fg_color.bind('<Button-1>', lambda e: self._pick_fg())
        self.btn_fg_color.pack(side='left',padx=3)
        self.btn_bg_color = tk.Label(r3,text='背景',relief='groove',
                                     font=('',8),cursor='hand2',padx=6,pady=2)
        self.btn_bg_color.bind('<Button-1>', lambda e: self._pick_bg())
        self.btn_bg_color.pack(side='left',padx=3)
        tk.Label(r3,text='⊕',font=('',11),cursor='hand2').pack(side='left',padx=1)
        sc_lbl = tk.Label(r3,text='取色',font=('',8),cursor='hand2',padx=4)
        sc_lbl.bind('<Button-1>', lambda e: self._screen_pick())
        sc_lbl.pack(side='left')
        rst_lbl = tk.Label(r3,text='↺重置',font=('',8),cursor='hand2',padx=6)
        rst_lbl.bind('<Button-1>', lambda e: self._reset_colors())
        rst_lbl.pack(side='left',padx=2)

        # 第四行：主题 + 快切 + 伪装
        r4 = row()
        tk.Label(r4,text='🌈 主题',font=('',8,'bold')).pack(side='left')
        self.var_theme = tk.StringVar(value=self.theme_name)
        cb = ttk.Combobox(r4,textvariable=self.var_theme,
                          values=list(self.THEMES.keys()),width=7,font=('',8),state='readonly')
        cb.pack(side='left',padx=4); cb.bind('<<ComboboxSelected>>',self._on_theme)

        for emoji, theme in [('☘','护眼'),('🌙','夜间'),('☀','暖黄')]:
            lbl = tk.Label(r4, text=emoji, font=('',12), cursor='hand2')
            lbl.pack(side='left', padx=2)
            lbl.bind('<Button-1>', lambda e, t=theme: (self.var_theme.set(t), self._on_theme()))

        word_lbl = tk.Label(r4,text='📝伪装',font=('',8),cursor='hand2',
                            padx=6,pady=2,relief='groove')
        word_lbl.bind('<Button-1>', lambda e: self.open_boss_editor())
        word_lbl.pack(side='left',padx=8)

        # 第五行：AI 模型
        r5 = row(pady=2)
        tk.Label(r5, text='🤖 AI',font=('',8,'bold')).pack(side='left')
        cb_ai = ttk.Combobox(r5, textvariable=self.var_ai_model,
                             values=['GLM (智谱)', 'MiniMax'], width=10, font=('',8), state='readonly')
        cb_ai.pack(side='left', padx=4)
        tk.Label(r5, text='· 快捷：Ctrl+D老板键  F5朗读  O书签  N笔记',
                 font=('',7), fg='#888').pack(side='left', padx=6)

        # 分隔线
        tk.Frame(self.setbar, height=1).pack(fill='x', padx=0, pady=2)
    ...
    def _pick_fg(self):
        t=self._cur_theme(); c=colorchooser.askcolor(color=t['fg'],title='字体颜色')
        if c and c[1]: self.THEMES[self.theme_name]['fg']=c[1]; self.txt.config(fg=c[1])
    ...
    def _apply_theme(self):
        t=self._cur_theme()
        bg,fg,bar,sel = t['bg'],t['fg'],t['bar'],t['sel']
        accent = t.get('accent', sel)
        card   = t.get('card', bar)

        self.root.configure(bg=bar)

        # 顶部栏
        self.topbar.config(bg=bar)
        self.lbl_title.config(bg=bar, fg=fg)
        for child in self.topbar.winfo_children():
            try: child.config(bg=bar)
            except: pass
            for grandchild in child.winfo_children():
                try: grandchild.config(bg=bar, fg=fg)
                except: pass

        # 图标按钮
        for btn in self._icon_btns:
            try: btn.config(bg=bar, fg=fg)
            except: pass

        # 进度条背景
        self.prog_canvas.config(bg=bar, highlightthickness=0)
        ...
        self.vsb.cv.config(bg=bg)
        self.vsb.set_colors(bg=bg, track=sel,
                            thumb=t.get('thumb',sel), thumb_hover=t.get('thumb_h',fg))
        self.txt.config(bg=bg, fg=fg, insertbackground=fg, selectbackground=sel)

        # 底部栏
        self.bot_area.config(bg=card)
        self.botbar.config(bg=card)
        self.lbl_prog.config(bg=card, fg=fg)
        for w in (self.btn_prev, self.btn_next, self.btn_tts, self.btn_note, self.btn_mark):
            try: w.config(bg=card, fg=fg)
            except: pass
    ...
    def _update_prog_bar(self):
        ...
        if not self.chapters: return
        t = self._cur_theme()
        pct = int((self.cur_ch / max(1, len(self.chapters)-1)) * 100)
        self.prog_canvas.update_idletasks()
        w = self.prog_canvas.winfo_width()
        if w < 2: return
        _draw_progress_bar(self.prog_canvas, pct,
                           t['bar'], t.get('accent', t['sel']), t['fg'], w, height=3)
    ...
    def _retheme_setbar(self):
        ...
        try:
            self.btn_fg_color.config(bg=t['fg'], fg=t['bg'])
            self.btn_bg_color.config(bg=t['bg'], fg=t['fg'])
        except: pass
    ...
    def open_file(self, path=None):
        ...
        self.book_path=path; self.book_title=title
        self.lbl_title.config(text=f'  {title[:18]}'); self.root.title(f'摸鱼阅读器 — {title}')
        self._load(text)
    ...
    def open_shelf(self):
        t=self._cur_theme(); bg,fg,bar,sel=t['bg'],t['fg'],t['bar'],t['sel']
        accent = t.get('accent', sel)
        win=tk.Toplevel(self.root); win.title('书架'); win.geometry('400x500')
        win.resizable(True,True); win.attributes('-topmost',True); win.configure(bg=bg)

        hdr=tk.Frame(win,bg=bar,height=36); hdr.pack(fill='x'); hdr.pack_propagate(False)
        tk.Label(hdr,text='  📚 我的书架',font=('',11,'bold'),bg=bar,fg=fg).pack(side='left',padx=10,pady=8)
        add_lbl = tk.Label(hdr,text='＋添加',font=('',9),bg=accent,fg='white',
                           padx=8,pady=3,cursor='hand2')
        add_lbl.bind('<Button-1>', lambda e: self._shelf_add(win))
        add_lbl.pack(side='right',padx=8,pady=8)
        tk.Label(hdr,text='×',font=('',13),bg=bar,fg=fg,cursor='hand2',padx=6).pack(side='right')

        frame=tk.Frame(win,bg=bg); frame.pack(fill='both',expand=True,padx=6,pady=6)
        sb2=tk.Scrollbar(frame,width=6); sb2.pack(side='right',fill='y')
        canvas=tk.Canvas(frame,bg=bg,highlightthickness=0,yscrollcommand=sb2.set)
        canvas.pack(fill='both',expand=True); sb2.config(command=canvas.yview)
        inner=tk.Frame(canvas,bg=bg); canvas.create_window((0,0),window=inner,anchor='nw')
        ...
        def refresh():
            ...
            if not self.shelf:
                tk.Label(inner,text='书架空空，快去添加书吧',font=('',10),bg=bg,fg=sel).pack(pady=30)
            for path,info in sorted(self.shelf.items(),key=lambda x:x[1].get('last_time',''),reverse=True):
                self._shelf_card(inner,path,info,bg,fg,bar,sel,accent,win,refresh)
            inner.update_idletasks()
            canvas.config(scrollregion=canvas.bbox('all'))
        refresh()

        rx=self.root.winfo_x()+self.root.winfo_width()+8; ry=self.root.winfo_y()
        win.geometry(f'400x500+{rx}+{ry}')

    def _shelf_card(self,parent,path,info,bg,fg,bar,sel,accent,shelf_win,refresh):
        card=tk.Frame(parent,bg=bar,relief='flat',bd=0)
        card.pack(fill='x',padx=6,pady=4)
        colors=['#8f3f71','#458588','#d79921','#689d6a','#cc241d','#3c78d8']
        ccolor=colors[hash(path)%len(colors)]
        cover=tk.Frame(card,bg=ccolor,width=40,height=56); cover.pack(side='left',padx=8,pady=8); cover.pack_propagate(False)
        tk.Label(cover,text='📖',font=('',16),bg=ccolor).pack(expand=True)
        info_f=tk.Frame(card,bg=bar); info_f.pack(side='left',fill='both',expand=True,pady=6)
        tk.Label(info_f,text=info.get('title','未知'),font=('',10,'bold'),bg=bar,fg=fg,anchor='w').pack(fill='x')
        lc=info.get('last_ch',0); tc=info.get('total_ch',1)
        pct=int(lc/max(1,tc-1)*100) if tc>1 else 0
        tk.Label(info_f,text=f'第 {lc+1} / {tc} 章  ·  {pct}%',font=('',8),bg=bar,fg=sel,anchor='w').pack(fill='x',pady=1)
        pb_f=tk.Canvas(info_f,height=4,bg=bar,highlightthickness=0)
        pb_f.pack(fill='x',pady=2)
        pb_f.update_idletasks()
        pb_w = max(1, pb_f.winfo_width())
        fill_w = max(0, int(pb_w * pct / 100))
        pb_f.create_rectangle(0,0,pb_w,4,fill=sel,outline='')
        if fill_w>0: pb_f.create_rectangle(0,0,fill_w,4,fill=ccolor,outline='')
        tk.Label(info_f,text=f'上次阅读：{info.get("last_time","")}',font=('',7),bg=bar,fg=sel,anchor='w').pack(fill='x')
        btn_f=tk.Frame(card,bg=bar); btn_f.pack(side='right',padx=8,pady=6)
        def open_it(p=path): shelf_win.destroy(); self.open_file(p)
        def del_it(p=path):
            if messagebox.askyesno('删除','从书架移除此书？',parent=shelf_win):
                del self.shelf[p]; _jsave(SHELF_FILE,self.shelf); refresh()
        read_lbl = tk.Label(btn_f,text='阅读',font=('',9,'bold'),bg=ccolor,fg='white',
                            padx=10,pady=4,cursor='hand2')
        read_lbl.bind('<Button-1>', lambda e: open_it())
        read_lbl.pack(pady=2)
        del_lbl = tk.Label(btn_f,text='移除',font=('',8),bg=bar,fg=sel,padx=6,cursor='hand2')
        del_lbl.bind('<Button-1>', lambda e: del_it())
        del_lbl.pack(pady=1)
    ...
    def _render_chapter(self, scroll_to_top=True):
        ...
        self._shelf_update()
        # 更新进度条
        self.root.after(50, self._update_prog_bar)
    ...
    def _update_nav(self):
        ...
        # 按钮状态用颜色区分
        t = self._cur_theme()
        self.btn_prev.config(fg=t['fg'] if self.cur_ch>0   else t['sel'])
        self.btn_next.config(fg=t['fg'] if self.cur_ch<n-1 else t['sel'])
    ...
    def toggle_tts(self):
        ...
        if self._tts_playing:
            self.tts.stop(); self._tts_playing=False
            self.btn_tts.config(text='🔊'); self._toast('⏹ 已停止朗读')
        else:
            if not self.chapters: return
            ch=self.chapters[self.cur_ch]
            self._tts_playing=True; self.btn_tts.config(text='⏸')
    ...
    def open_marks(self):
        marks=self.marks.get(self.book_path,[])
        t=self._cur_theme(); bg,fg,bar,sel=t['bg'],t['fg'],t['bar'],t['sel']
        win=tk.Toplevel(self.root); win.title('书签'); win.geometry('320x380')
        win.resizable(True,True); win.attributes('-topmost',True); win.configure(bg=bg)
        hdr=tk.Frame(win,bg=bar,height=32); hdr.pack(fill='x'); hdr.pack_propagate(False)
        tk.Label(hdr,text='  🔖 书签列表',font=('',10,'bold'),bg=bar,fg=fg).pack(side='left',padx=8,pady=6)
        tk.Label(hdr,text='×',font=('',12),bg=bar,fg=fg,cursor='hand2',padx=8).pack(side='right')
        lf=tk.Frame(win,bg=bg); lf.pack(fill='both',expand=True,padx=6,pady=6)
        sb2=tk.Scrollbar(lf,width=6); sb2.pack(side='right',fill='y')
        lb=tk.Listbox(lf,font=('',10),relief='flat',bg=bg,fg=fg,selectbackground=sel,
                      selectforeground=fg,borderwidth=0,highlightthickness=0,
                      activestyle='none',yscrollcommand=sb2.set)
        lb.pack(fill='both',expand=True); sb2.config(command=lb.yview)
        rows=list(marks)
        for m in rows: lb.insert('end',f"  🔖  第{m['ch']+1}章  {m['ch_title'][:16]}  ·  {m['time']}")
        bf=tk.Frame(win,bg=bar); bf.pack(fill='x',padx=8,pady=6)
        def jump():
            idxs=lb.curselection()
            if not idxs: return
            win.destroy(); self.goto_chapter(rows[idxs[0]]['ch'])
        def delete():
            ...
            self.marks[self.book_path]=[x for x in self.marks.get(self.book_path,[]) if x is not m]
            _jsave(MARKS_FILE,self.marks); self._update_nav()
        jmp_lbl = tk.Label(bf,text='↩ 跳转',font=('',9,'bold'),bg=t.get('accent',sel),fg='white',
                           padx=10,pady=4,cursor='hand2')
        jmp_lbl.bind('<Button-1>', lambda e: jump())
        jmp_lbl.pack(side='left',padx=4)
        del_lbl = tk.Label(bf,text='🗑 删除',font=('',9),bg=bar,fg=fg,padx=8,cursor='hand2')
        del_lbl.bind('<Button-1>', lambda e: delete())
        del_lbl.pack(side='left')
        lb.bind('<Double-Button-1>',lambda e:jump())
        rx=self.root.winfo_x()+self.root.winfo_width()+8; ry=self.root.winfo_y()
        win.geometry(f'320x380+{rx}+{ry}')
    ...
    def add_note(self):
        ...
        ch=self.chapters[self.cur_ch]
        t=self._cur_theme(); bg,fg,bar,sel=t['bg'],t['fg'],t['bar'],t['sel']
        win=tk.Toplevel(self.root); win.title('添加笔记'); win.geometry('360x280')
        win.attributes('-topmost',True); win.configure(bg=bg)
        hdr=tk.Frame(win,bg=bar,height=32); hdr.pack(fill='x'); hdr.pack_propagate(False)
        tk.Label(hdr,text=f'  ✏ 笔记 — {ch["title"][:20]}',font=('',9,'bold'),bg=bar,fg=fg).pack(side='left',padx=8,pady=6)
        tk.Label(hdr,text='×',font=('',12),bg=bar,fg=fg,cursor='hand2',padx=8).pack(side='right')
        sel_text=''
        try: sel_text=self.txt.get(tk.SEL_FIRST,tk.SEL_LAST)
        except: pass
        if sel_text:
            ref=tk.Label(win,text=sel_text[:80]+'…' if len(sel_text)>80 else sel_text,
                        font=('',8,'italic'),bg=sel,fg=fg,wraplength=320,anchor='w',justify='left',padx=8,pady=4)
            ref.pack(fill='x',padx=10,pady=(8,0))
        ta=tk.Text(win,height=6,font=('',10),bg=bg,fg=fg,insertbackground=fg,
                   relief='flat',padx=10,pady=6,wrap='word',borderwidth=1,highlightthickness=1,
                   highlightbackground=sel,highlightcolor=t.get('accent',fg))
        ta.pack(fill='both',expand=True,padx=10,pady=6); ta.focus()
        def save():
            content=ta.get('1.0','end').strip()
            if not content: win.destroy(); return
            notes=self.notes.setdefault(self.book_path,[])
            ...
            _jsave(NOTES_FILE,self.notes); self._update_nav(); win.destroy()
            self._toast('✏ 笔记已保存')
        bf=tk.Frame(win,bg=bar); bf.pack(fill='x',padx=10,pady=6)
        save_lbl = tk.Label(bf,text='保存笔记 (Ctrl+↵)',font=('',9,'bold'),
                            bg=t.get('accent',sel),fg='white',padx=12,pady=4,cursor='hand2')
        save_lbl.bind('<Button-1>', lambda e: save())
        save_lbl.pack(side='right',padx=4)
        win.bind('<Control-Return>',lambda e:save())

    def open_notes(self):
        notes=self.notes.get(self.book_path,[])
        t=self._cur_theme(); bg,fg,bar,sel=t['bg'],t['fg'],t['bar'],t['sel']
        win=tk.Toplevel(self.root); win.title('笔记'); win.geometry('380x500')
        win.resizable(True,True); win.attributes('-topmost',True); win.configure(bg=bg)
        hdr=tk.Frame(win,bg=bar,height=32); hdr.pack(fill='x'); hdr.pack_propagate(False)
        tk.Label(hdr,text='  ✏ 笔记列表',font=('',10,'bold'),bg=bar,fg=fg).pack(side='left',padx=8,pady=6)
        tk.Label(hdr,text='×',font=('',12),bg=bar,fg=fg,cursor='hand2',padx=8).pack(side='right')
        paned=tk.PanedWindow(win,orient='vertical',bg=bg,sashwidth=5,sashrelief='flat')
        paned.pack(fill='both',expand=True,padx=4,pady=4)
        top_f=tk.Frame(paned,bg=bg); paned.add(top_f,height=200)
        sb2=tk.Scrollbar(top_f,width=6); sb2.pack(side='right',fill='y')
        lb=tk.Listbox(top_f,font=('',9),relief='flat',bg=bg,fg=fg,selectbackground=sel,
                      selectforeground=fg,borderwidth=0,highlightthickness=0,
                      activestyle='none',yscrollcommand=sb2.set)
        lb.pack(fill='both',expand=True); sb2.config(command=lb.yview)
        ...
        def delete():
            ...
            _jsave(NOTES_FILE,self.notes); self._update_nav()
            detail.config(state='normal'); detail.delete('1.0','end'); detail.config(state='disabled')
        jmp_lbl = tk.Label(bf,text='↩ 跳转章节',font=('',9,'bold'),
                           bg=t.get('accent',sel),fg='white',padx=10,pady=4,cursor='hand2')
        jmp_lbl.bind('<Button-1>', lambda e: jump())
        jmp_lbl.pack(side='left',padx=4)
        del_lbl = tk.Label(bf,text='🗑 删除',font=('',9),bg=bar,fg=fg,padx=8,cursor='hand2')
        del_lbl.bind('<Button-1>', lambda e: delete())
    ...
    def open_chapters(self):
        ...
        mark_set={m['ch'] for m in self.marks.get(self.book_path,[])}
        note_set={n2['ch'] for n2 in self.notes.get(self.book_path,[])}
        win=tk.Toplevel(self.root); win.title('章节目录'); win.geometry('320x520')
        win.resizable(True,True); win.attributes('-topmost',True); win.configure(bg=bg)
        hdr=tk.Frame(win,bg=bar,height=32); hdr.pack(fill='x'); hdr.pack_propagate(False)
        tk.Label(hdr,text='  📑 章节目录',font=('',10,'bold'),bg=bar,fg=fg).pack(side='left',padx=8,pady=6)
        tk.Label(hdr,text='×',font=('',12),bg=bar,fg=fg,cursor='hand2',padx=8).pack(side='right')
        sf=tk.Frame(win,bg=bg); sf.pack(fill='x',padx=8,pady=(8,4))
        sv=tk.StringVar()
        se=tk.Entry(sf,textvariable=sv,font=('',10),bg=bg,fg=fg,insertbackground=fg,
                    relief='flat',borderwidth=1,highlightthickness=1,
                    highlightbackground=sel,highlightcolor=accent)
        se.pack(fill='x',ipady=4)
        PH='🔍 搜索章节名…'
        se.insert(0,PH); se.config(fg='gray')
        se.bind('<FocusIn>',lambda e:(se.delete(0,'end'),se.config(fg=fg)) if se.get()==PH else None)
        se.bind('<FocusOut>',lambda e:(se.insert(0,PH),se.config(fg='gray')) if not se.get() else None)
        lf=tk.Frame(win,bg=bg); lf.pack(fill='both',expand=True,padx=6,pady=4)
        sb2=tk.Scrollbar(lf,width=6); sb2.pack(side='right',fill='y')
        lb=tk.Listbox(lf,font=('',10),relief='flat',bg=bg,fg=fg,selectbackground=sel,
                      selectforeground=fg,borderwidth=0,highlightthickness=0,
                      activestyle='none',yscrollcommand=sb2.set)
        lb.pack(fill='both',expand=True); sb2.config(command=lb.yview)
        jmp_lbl=tk.Label(win,text='↩ 跳转到此章',font=('',10,'bold'),
                         bg=accent,fg='white',pady=7,cursor='hand2')
        jmp_lbl.pack(fill='x',padx=8,pady=6)
        all_ch=list(self.chapters); visible=list(range(len(all_ch)))
        def fill(indices):
            visible.clear(); visible.extend(indices); lb.delete(0,'end')
            for i in indices:
        ...
        def jump(e=None):
            ...
            if not idxs: return
            win.destroy(); self.goto_chapter(visible[idxs[0]])
        jmp_lbl.bind('<Button-1>', jump)
        lb.bind('<Double-Button-1>',jump); lb.bind('<Return>',jump)
        rx=self.root.winfo_x()+self.root.winfo_width()+8; ry=self.root.winfo_y()
        win.geometry(f'320x520+{rx}+{ry}')

    # ─────────────────────────────────────────────────
    # Toast 提示
    # ─────────────────────────────────────────────────
    def _toast(self,msg,ms=1600):
        t=self._cur_theme(); accent=t.get('accent',t['sel'])
        try:
            for w in self.root.winfo_children():
                if isinstance(w,tk.Label) and getattr(w,'_is_toast',False): w.destroy()
        except: pass
        toast=tk.Label(self.root,text=f'  {msg}  ',font=('',9),
                       bg=accent,fg='white',padx=8,pady=5,relief='flat')
        toast._is_toast=True
        toast.place(relx=0.5,rely=0.07,anchor='n')
        self.root.after(ms,lambda:(toast.winfo_exists() and toast.destroy()))
    ...
    def toggle_minimize(self):
        ...
        if self._minimized:
            self._saved_h=self.root.winfo_height()
            for w in (self.setbar,self.sep,self.txt_frame,self.bot_area,self.prog_canvas): w.pack_forget()
            self.root.geometry(f'{self.root.winfo_width()}x44')
        else:
            self.root.geometry(f'{self.root.winfo_width()}x{self._saved_h}')
            self.prog_canvas.pack(fill='x',after=self.topbar)
            self.sep.pack(fill='x',after=self.topbar)
            if self._settings_open: self.setbar.pack(fill='x',after=self.topbar); self._retheme_setbar()
            self.txt_frame.pack(fill='both',expand=True)
            self.bot_area.pack(fill='x',side='bottom')
    ...
    def open_search(self):
        ...
        if hasattr(self,'_search_win') and self._search_win.winfo_exists():
            self._search_win.lift(); self._search_win.focus_force(); return
        win=tk.Toplevel(self.root); win.title('全文搜索'); win.geometry('500x580')
        win.resizable(True,True); win.attributes('-topmost',True); win.configure(bg=bg)
        self._search_win=win
        top=tk.Frame(win,bg=bar,height=40); top.pack(fill='x'); top.pack_propagate(False)
        tk.Label(top,text='🔍',font=('',12),bg=bar,fg=fg).pack(side='left',padx=10,pady=8)
        sv=tk.StringVar()
        entry=tk.Entry(top,textvariable=sv,font=('',11),bg=bg,fg=fg,
                       insertbackground=fg,relief='flat',bd=0)
        entry.pack(side='left',fill='x',expand=True,ipady=6)
        var_case=tk.BooleanVar(value=False); var_regex=tk.BooleanVar(value=False)
        tk.Checkbutton(top,text='Aa',variable=var_case,font=('',8),bg=bar,fg=fg,
                       activebackground=bar,selectcolor=bg).pack(side='left',padx=2)
        tk.Checkbutton(top,text='.*',variable=var_regex,font=('',8),bg=bar,fg=fg,
                       activebackground=bar,selectcolor=bg).pack(side='left',padx=2)
        lbl_count=tk.Label(top,text='',font=('',8),bg=bar,fg=sel); lbl_count.pack(side='left',padx=6)
        tk.Label(top,text='×',font=('',12),bg=bar,fg=fg,cursor='hand2',padx=8).pack(side='right')
        tk.Frame(win,height=1,bg=sel).pack(fill='x')
        list_frame=tk.Frame(win,bg=bg); list_frame.pack(fill='both',expand=True)
        sb2=tk.Scrollbar(list_frame,width=6); sb2.pack(side='right',fill='y')
        lb=tk.Listbox(list_frame,font=('',9),relief='flat',bg=bg,fg=fg,
                      selectbackground=sel,selectforeground=fg,
                      borderwidth=0,highlightthickness=0,activestyle='none',yscrollcommand=sb2.set)
        lb.pack(fill='both',expand=True); sb2.config(command=lb.yview)
        tk.Frame(win,height=1,bg=sel).pack(fill='x')
        preview_frame=tk.Frame(win,bg=bg,height=130); preview_frame.pack(fill='x'); preview_frame.pack_propagate(False)
        preview=tk.Text(preview_frame,font=('',9),bg=bg,fg=fg,relief='flat',padx=14,pady=8,wrap='word',
                        state='disabled',borderwidth=0,highlightthickness=0)
        preview.pack(fill='both',expand=True)
        preview.tag_config('hit',background=accent,foreground='white')
        preview.tag_config('info',foreground=sel)
        bot_f=tk.Frame(win,bg=bar,height=36); bot_f.pack(fill='x'); bot_f.pack_propagate(False)
        lbl_loc=tk.Label(bot_f,text='',font=('',8),bg=bar,fg=fg); lbl_loc.pack(side='left',padx=10,pady=8)
        btn_jump=tk.Label(bot_f,text='↩ 跳转到此处',font=('',9,'bold'),
                          bg=accent,fg='white',padx=10,pady=5,cursor='hand2')
        btn_jump.pack(side='right',padx=10,pady=5)
        results=[]
        def do_search(*_):
            results.clear(); lb.delete(0,'end')
            query=sv.get().strip()
            if not query: lbl_count.config(text=''); return
            flags=0 if var_case.get() else re.IGNORECASE
            try:
                pat=re.compile(query if var_regex.get() else re.escape(query),flags)
            except re.error as e: lbl_count.config(text=f'正则错误: {e}'); return
            CONTEXT=40
            for ci,ch in enumerate(self.chapters):
                for m in pat.finditer(ch['body']):
                    s,e2=m.start(),m.end()
                    ...
                    hit=ch['body'][s:e2]
                    post=ch['body'][e2:min(len(ch['body']),e2+CONTEXT)].replace('\n',' ')
                    results.append((ci,s,e2,(pre,hit,post)))
            lbl_count.config(text=f'共 {len(results)} 处')
            for ci,s,e2,(pre,hit,post) in results:
                lb.insert('end',f"  第{ci+1}章  {self.chapters[ci]['title'][:14]}  「{hit[:20]}」")
            preview.config(state='normal'); preview.delete('1.0','end'); preview.config(state='disabled')
            lbl_loc.config(text='')
        sv.trace_add('write',do_search); var_case.trace_add('write',do_search); var_regex.trace_add('write',do_search)
        def on_select(event=None):
            idxs=lb.curselection()
            if not idxs: return
            ci,s,e2,(pre,hit,post)=results[idxs[0]]
            ...
            preview.config(state='disabled')
            lbl_loc.config(text=f'第{ci+1}章 · 位置 {s}')
            btn_jump.config(command=lambda:_jump(ci,s,e2,hit))
        def _jump(ci,s,e2,hit):
            self.goto_chapter(ci); win.lift()
            self.root.after(80,lambda:_hl(hit))
        def _hl(hit):
            self.txt.tag_remove('search_hit','1.0','end')
            self.txt.tag_config('search_hit',background=accent,foreground='white')
            start='1.0'
            while True:
                pos=self.txt.search(hit,start,nocase=not var_case.get(),stopindex='end')
                if not pos: break
                end_pos=f'{pos}+{len(hit)}c'
                self.txt.tag_add('search_hit',pos,end_pos); self.txt.see(pos); start=end_pos
        lb.bind('<<ListboxSelect>>',on_select)
        lb.bind('<Double-Button-1>',lambda e:btn_jump.invoke() if hasattr(btn_jump,'invoke') else None)
        entry.focus()
        try:
            sel_text=self.txt.get(tk.SEL_FIRST,tk.SEL_LAST).strip()
            if sel_text and '\n' not in sel_text: sv.set(sel_text)
        except: pass
        rx=self.root.winfo_x()+self.root.winfo_width()+8; ry=self.root.winfo_y()
        win.geometry(f'500x580+{rx}+{ry}')
    ...
    def open_summary(self):
        ...
        st=State()
        _t=self._cur_theme(); _bg,_fg,_bar,_sel=_t['bg'],_t['fg'],_t['bar'],_t['sel']
        _accent=_t.get('accent',_sel)

        dlg=tk.Toplevel(self.root); dlg.title('章节深度分析')
        dlg.geometry('520x600'); dlg.resizable(True,True)
        dlg.attributes('-topmost',True); dlg.configure(bg=_bg)

        top=tk.Frame(dlg,bg=_bar,height=38); top.pack(fill='x'); top.pack_propagate(False)
        _cc=len(st.ch_body); _pc=len([p for p in st.ch_body.split('\n') if p.strip()])
        tk.Label(top,text=f'  ✨  {st.ch_title}',font=('',10,'bold'),
                 bg=_bar,fg=_fg,anchor='w').pack(side='left',fill='x',expand=True,padx=8,pady=9)
        tk.Label(top,text=f'{_cc}字  {_pc}段',font=('',8),bg=_bar,fg=_sel).pack(side='right',padx=8)
        tk.Label(top,text='×',font=('',13),bg=_bar,fg=_fg,cursor='hand2',padx=8).pack(side='right')
        tk.Frame(dlg,height=1,bg=_sel).pack(fill='x')

        # ── 分析维度选择（核心升级）────────────────────
        dim_frame=tk.Frame(dlg,bg=_bar); dim_frame.pack(fill='x',padx=10,pady=6)
        tk.Label(dim_frame,text='分析维度：',font=('',9,'bold'),bg=_bar,fg=_fg).pack(side='left')
        ...
        dim_vars = {}
        for icon_lbl, key, default in DIMS:
            var = tk.BooleanVar(value=default)
            dim_vars[key] = var
            cb = tk.Checkbutton(dim_frame,text=icon_lbl,variable=var,font=('',8),
                                bg=_bar,fg=_fg,activebackground=_bar,selectcolor=_bg,
                                cursor='hand2')
            cb.pack(side='left',padx=3)

        # 风格行
        sty_frame=tk.Frame(dlg,bg=_bar); sty_frame.pack(fill='x',padx=10,pady=2)
        tk.Label(sty_frame,text='篇幅：',font=('',9,'bold'),bg=_bar,fg=_fg).pack(side='left')
        v_len=tk.StringVar(value='标准')
        for lbl in ['简短','标准','详细']:
            tk.Radiobutton(sty_frame,text=lbl,variable=v_len,value=lbl,
                           font=('',8),bg=_bar,fg=_fg,activebackground=_bar,
                           selectcolor=_bg).pack(side='left',padx=4)
        tk.Label(sty_frame,text='  风格：',font=('',9,'bold'),bg=_bar,fg=_fg).pack(side='left')
        v_sty=tk.StringVar(value='客观')
        for lbl in ['客观','活泼','学术']:
            tk.Radiobutton(sty_frame,text=lbl,variable=v_sty,value=lbl,
                           font=('',8),bg=_bar,fg=_fg,activebackground=_bar,
                           selectcolor=_bg).pack(side='left',padx=4)
        tk.Frame(dlg,height=1,bg=_sel).pack(fill='x')

        mid=tk.Frame(dlg,bg=_bg); mid.pack(fill='both',expand=True)
        _vsb2=tk.Scrollbar(mid,width=6); _vsb2.pack(side='right',fill='y')
        out=tk.Text(mid,font=(self.font_fam,self.font_size),
                    bg=_bg,fg=_fg,insertbackground=_fg,selectbackground=_sel,
                    relief='flat',padx=18,pady=14,wrap='word',
                    borderwidth=0,highlightthickness=0,state='disabled',
                    yscrollcommand=_vsb2.set,spacing1=4,spacing3=4)
        out.pack(side='left',fill='both',expand=True)
        _vsb2.config(command=out.yview)
        ...
        out.tag_config('warn',    foreground='#cc241d')
        out.tag_config('loading', foreground=_sel,font=(self.font_fam,self.font_size,'italic'))
        out.tag_config('result',  foreground=_fg,font=(self.font_fam,self.font_size))
        out.tag_config('section_title', foreground=_accent,
                       font=(self.font_fam,self.font_size+1,'bold'))
        tk.Frame(dlg,height=1,bg=_sel).pack(fill='x')

        bot=tk.Frame(dlg,bg=_bar,height=48); bot.pack(fill='x',side='bottom'); bot.pack_propagate(False)
        btn_gen=tk.Label(bot,text='▶  开始深度分析',font=('',10,'bold'),
                         bg=_accent,fg='white',padx=18,pady=6,cursor='hand2')
        btn_gen.pack(side='left',padx=12,pady=10)
        btn_copy=tk.Label(bot,text='复制',font=('',9),bg=_bar,fg=_fg,padx=8,cursor='hand2')
        btn_copy.pack(side='left',padx=3,pady=10)
        btn_note=tk.Label(bot,text='存为笔记',font=('',9),bg=_bar,fg=_fg,padx=8,cursor='hand2')
        btn_note.pack(side='left',padx=3,pady=10)
        lbl_st=tk.Label(bot,text='',font=('',8),bg=_bar,fg=_sel); lbl_st.pack(side='right',padx=12)

        def _show(text,tag='result'):
            out.config(state='normal'); out.delete('1.0','end')
            out.insert('end',text,tag); out.config(state='disabled'); out.yview_moveto(0.0)

        def _show_structured(text):
            """将多维度分析结果用分段样式渲染"""
            out.config(state='normal'); out.delete('1.0','end')
            for line in text.split('\n'):
                stripped=line.strip()
                if stripped.startswith('【') and '】' in stripped:
                    out.insert('end',stripped+'\n','section_title')
                elif stripped.startswith('▶') or stripped.startswith('•'):
                    out.insert('end',stripped+'\n','result')
                else:
                    out.insert('end',line+'\n','result')
            out.config(state='disabled'); out.yview_moveto(0.0)

        # ★ 核心：5维度结构化 Prompt ★
        def _build_5dim_prompt():
            selected_dims=[k for k,v in dim_vars.items() if v.get()]
            if not selected_dims: selected_dims=['主题思想','人物弧']

            _len_map={'简短':'每个维度50字内','标准':'每个维度100-150字','详细':'每个维度200字以上'}
            _sty_map={'客观':'语言客观简练','活泼':'语言生动，适当使用emoji','学术':'语言严谨，使用文学术语'}

            dim_instructions=[]
            dim_prompts={
                '主题思想': '【🎯 主题思想】\n分析本章的核心主题和思想深度，作者想传递什么？',
                '人物弧':   '【👤 人物弧】\n分析本章人物的内心变化、成长或退化，有哪些关键行为体现了性格？',
                '伏笔悬念': '【🧩 伏笔悬念】\n指出本章埋下的伏笔或留下的悬念，以及本章揭开了哪些之前的谜题？',
                '高潮转折': '【⚡ 高潮转折】\n分析本章的情节高潮点、冲突和转折，最戏剧性的时刻是什么？',
                '金句摘录': '【💎 金句摘录】\n摘录1-3句最精彩的原文句子，并简析其文学价值或情感张力。',
            }
            for d in selected_dims:
                if d in dim_prompts: dim_instructions.append(dim_prompts[d])

            clip=st.ch_body[:5000]
            return (
                f"请对以下小说章节进行结构化深度分析。\n"
                f"要求：{_len_map[v_len.get()]}，{_sty_map[v_sty.get()]}。\n"
                f"按以下维度逐一分析，每个维度单独一段：\n\n"
                + '\n\n'.join(dim_instructions) +
                f"\n\n正文：\n【{st.ch_title}】\n{clip}"
            )

        def _on_ok(text):
            st.result=text; st.running=False
            _show_structured(text)
            btn_gen.config(text='↺  重新分析')
            lbl_st.config(text=f'已生成 · {len(text)}字')

        def _on_err(msg):
            st.running=False; btn_gen.config(text='▶  开始深度分析')
            if msg.startswith('NO_KEY_'):
                _model=msg.replace('NO_KEY_','')
                _fname='api_key_minimax.txt' if _model=='MiniMax' else 'api_key.txt'
                _show(f'⚠  未找到 {_model} API Key\n请新建文件：{_fname}','warn')
            else: _show(f'⚠  生成失败\n\n{msg}','warn')

        def _do_gen():
            if st.running: return
            selected=[k for k,v in dim_vars.items() if v.get()]
            if not selected: self._toast('请至少选择一个分析维度'); return
            st.running=True; lbl_st.config(text='')
            btn_gen.config(text='分析中…')
            _model_choice=self.var_ai_model.get()
            _show(f'[{_model_choice}] 正在进行五维度深度分析，请稍候…','loading')
            def worker():
                import urllib.request as _ureq, urllib.error as _uerr, json as _uj
                _key=_get_api_key(_model_choice)
                if not _key: dlg.after(0,lambda:_on_err(f'NO_KEY_{_model_choice}')); return
                _url='https://api.minimax.chat/v1/chat/completions' if _model_choice=='MiniMax' else 'https://open.bigmodel.cn/api/paas/v4/chat/completions'
                _model_name='abab6.5s-chat' if _model_choice=='MiniMax' else 'glm-4-flash'
                _tok_map={'简短':300,'标准':800,'详细':1500}
                _prompt=_build_5dim_prompt()
                _payload=_uj.dumps({'model':_model_name,'max_tokens':_tok_map.get(v_len.get(),800),
                                    'temperature':0.65,'messages':[{'role':'user','content':_prompt}]}).encode('utf-8')
                _req=_ureq.Request(_url,data=_payload,
                                   headers={'Content-Type':'application/json','Authorization':f'Bearer {_key}'},
                                   method='POST')
                try:
                    with _ureq.urlopen(_req,timeout=40) as _resp:
                        _raw=_resp.read().decode('utf-8')
                        _text=_uj.loads(_raw)['choices'][0]['message']['content'].strip()
                    dlg.after(0,lambda t=_text:_on_ok(t))
                except _uerr.HTTPError as _he:
                    _hbody=_he.read().decode('utf-8',errors='replace')
                    try: _hemsg=_uj.loads(_hbody).get('error',{}).get('message',_hbody)
                    except: _hemsg=_hbody[:200]
                    dlg.after(0,lambda c=str(_he.code),m=_hemsg:_on_err(f'HTTP {c}：{m}'))
                except Exception as _exc: dlg.after(0,lambda m=str(_exc):_on_err(m))
            threading.Thread(target=worker,daemon=True).start()

        btn_gen.bind('<Button-1>',lambda e:_do_gen())
        btn_copy.bind('<Button-1>',lambda e:(dlg.clipboard_clear(),dlg.clipboard_append(st.result),lbl_st.config(text='已复制 ✓')) if st.result else None)
        btn_note.bind('<Button-1>',lambda e:self._save_summary_as_note(st,v_len,v_sty,dlg,lbl_st) if st.result else None)

        _show('选择分析维度（可多选），点击「▶ 开始深度分析」。\n\n维度说明：\n'
              '🎯 主题思想  ·  👤 人物弧  ·  🧩 伏笔悬念\n'
              '⚡ 高潮转折  ·  💎 金句摘录\n\n'
              '可组合任意维度，AI会逐维度深度解析。','hint')

        _rx=self.root.winfo_x()+self.root.winfo_width()+10; _ry=self.root.winfo_y()
        dlg.geometry(f'520x600+{_rx}+{_ry}')

    def _save_summary_as_note(self,st,v_len,v_sty,dlg,lbl_st):
        _nl=self.notes.setdefault(self.book_path,[])
        _nl.append({'ch':st.ch_idx,'ch_title':st.ch_title,
                    'content':f'[AI深度分析·{v_len.get()}·{v_sty.get()}]\n{st.result}',
                    'quote':'','time':datetime.datetime.now().strftime('%m-%d %H:%M')})
        _jsave(NOTES_FILE,self.notes); self._update_nav()
        self._toast('✏ 分析已存为笔记'); lbl_st.config(text='已保存为笔记 ✓')

    # ─────────────────────────────────────────────────
    # ★ 问题3-方案A：TF-IDF 粗筛 + AI 精排 双阶段查找 ★
    # ─────────────────────────────────────────────────
    def open_chapter_finder(self):
        if not self.chapters: messagebox.showinfo('提示','请先打开小说'); return
        ...
        dlg=tk.Toplevel(self.root); dlg.title('AI 智能章节查找')
        dlg.geometry('520x660'); dlg.resizable(True,True)
        dlg.attributes('-topmost',True); dlg.configure(bg=_bg)

        top=tk.Frame(dlg,bg=_bar,height=38); top.pack(fill='x'); top.pack_propagate(False)
        tk.Label(top,text='  🔮  AI 智能章节查找  （两阶段检索）',
                 font=('',10,'bold'),bg=_bar,fg=_fg).pack(side='left',padx=8,pady=9)
        tk.Label(top,text=f'{total_ch}章',font=('',8),bg=_bar,fg=_sel).pack(side='right',padx=8)
        tk.Label(top,text='×',font=('',13),bg=_bar,fg=_fg,cursor='hand2',padx=8).pack(side='right')
        tk.Frame(dlg,height=1,bg=_sel).pack(fill='x')

        inp_wrap=tk.Frame(dlg,bg=_bg); inp_wrap.pack(fill='x',padx=12,pady=(10,0))
        tk.Label(inp_wrap,text='描述你记得的情节、场景、人物、对话或感受：',
                 font=('',9),bg=_bg,fg=_fg,anchor='w').pack(fill='x')
        inp=tk.Text(inp_wrap,font=(self.font_fam,self.font_size),
                    bg=_bar,fg=_fg,insertbackground=_fg,relief='flat',padx=10,pady=8,
                    wrap='word',height=4,borderwidth=1,highlightthickness=1,
                    highlightbackground=_sel,highlightcolor=_accent)
        inp.pack(fill='x',pady=(4,0))
        _PH='例如：主角第一次失败，独自在雨中走路，背景音乐很悲伤……'
        inp.insert('1.0',_PH); inp.config(fg=_sel)
        def _fi(e):
            if inp.get('1.0','end-1c')==_PH: inp.delete('1.0','end'); inp.config(fg=_fg)
        def _fo(e):
            if not inp.get('1.0','end-1c').strip(): inp.insert('1.0',_PH); inp.config(fg=_sel)
        inp.bind('<FocusIn>',_fi); inp.bind('<FocusOut>',_fo)

        # ── 两阶段参数配置（核心展示）────────────────
        cfg_frame=tk.Frame(inp_wrap,bg=_bg); cfg_frame.pack(fill='x',pady=(8,0))

        # 阶段一参数
        tk.Label(cfg_frame,text='阶段①粗筛：',font=('',8,'bold'),bg=_bg,fg=_fg).pack(side='left')
        tk.Label(cfg_frame,text='TF-IDF取前',font=('',8),bg=_bg,fg=_sel).pack(side='left')
        v_tfidf_n=tk.IntVar(value=20)
        sp1=ttk.Combobox(cfg_frame,textvariable=v_tfidf_n,values=[10,15,20,30,50],
                         width=3,font=('',8),state='readonly')
        sp1.pack(side='left',padx=2)
        tk.Label(cfg_frame,text='章',font=('',8),bg=_bg,fg=_sel).pack(side='left')

        tk.Label(cfg_frame,text='   阶段②AI精排：',font=('',8,'bold'),bg=_bg,fg=_fg).pack(side='left')
        tk.Label(cfg_frame,text='返回',font=('',8),bg=_bg,fg=_sel).pack(side='left')
        v_cnt=tk.IntVar(value=3)
        sp2=ttk.Combobox(cfg_frame,textvariable=v_cnt,values=[1,3,5,7],
                         width=3,font=('',8),state='readonly')
        sp2.pack(side='left',padx=2)
        tk.Label(cfg_frame,text='个结果',font=('',8),bg=_bg,fg=_sel).pack(side='left')

        # 阶段进度指示器
        stage_frame=tk.Frame(inp_wrap,bg=_bg); stage_frame.pack(fill='x',pady=(6,0))
        lbl_s1=tk.Label(stage_frame,text='① 本地TF-IDF粗筛',font=('',8),bg=_bg,fg=_sel)
        lbl_s1.pack(side='left')
        tk.Label(stage_frame,text=' → ',font=('',9),bg=_bg,fg=_sel).pack(side='left')
        lbl_s2=tk.Label(stage_frame,text='② AI语义精排',font=('',8),bg=_bg,fg=_sel)
        lbl_s2.pack(side='left')
        tk.Label(stage_frame,text=' → ',font=('',9),bg=_bg,fg=_sel).pack(side='left')
        lbl_s3=tk.Label(stage_frame,text='输出结果',font=('',8),bg=_bg,fg=_sel)
        lbl_s3.pack(side='left')

        tk.Frame(dlg,height=1,bg=_sel).pack(fill='x',pady=(10,0))

        pane=tk.PanedWindow(dlg,orient='vertical',bg=_bg,sashwidth=5,sashrelief='flat',sashpad=2)
        pane.pack(fill='both',expand=True)
        list_frame=tk.Frame(pane,bg=_bg); pane.add(list_frame,height=180)
        _lsb=tk.Scrollbar(list_frame,width=6); _lsb.pack(side='right',fill='y')
        result_lb=tk.Listbox(list_frame,font=('',10),relief='flat',bg=_bg,fg=_fg,
                             selectbackground=_sel,selectforeground=_fg,
                             borderwidth=0,highlightthickness=0,activestyle='none',
                             yscrollcommand=_lsb.set)
        result_lb.pack(fill='both',expand=True); _lsb.config(command=result_lb.yview)
        detail_frame=tk.Frame(pane,bg=_bg); pane.add(detail_frame,height=150)
        detail=tk.Text(detail_frame,font=(self.font_fam,self.font_size),
                       bg=_bar,fg=_fg,insertbackground=_fg,relief='flat',
                       padx=14,pady=10,wrap='word',borderwidth=0,highlightthickness=0,state='disabled')
        detail.pack(fill='both',expand=True)
        detail.tag_config('title',font=(self.font_fam,self.font_size,'bold'))
        detail.tag_config('score',foreground=_sel)
        detail.tag_config('phase',foreground=_accent,font=(self.font_fam,self.font_size-1,'italic'))
        detail.tag_config('hint',foreground=_sel,font=(self.font_fam,self.font_size,'italic'))
        detail.tag_config('warn',foreground='#cc241d')
        tk.Frame(dlg,height=1,bg=_sel).pack(fill='x')

        bot=tk.Frame(dlg,bg=_bar,height=46); bot.pack(fill='x',side='bottom'); bot.pack_propagate(False)
        btn_find=tk.Label(bot,text='🔮  开始两阶段检索',font=('',10,'bold'),
                          bg=_accent,fg='white',padx=16,pady=5,cursor='hand2')
        btn_find.pack(side='left',padx=12,pady=8)
        btn_goto=tk.Label(bot,text='↩ 跳转到此章',font=('',9),
                          bg=_bar,fg=_fg,padx=8,cursor='hand2')
        btn_goto.pack(side='left',padx=4,pady=8)
        lbl_st2=tk.Label(bot,text='',font=('',8),bg=_bar,fg=_sel); lbl_st2.pack(side='right',padx=12)

        def _detail_show(text,tag='hint'):
            detail.config(state='normal'); detail.delete('1.0','end')
            detail.insert('end',text,tag); detail.config(state='disabled')

        def _detail_rich(title,score,reason,phase_info=''):
            detail.config(state='normal'); detail.delete('1.0','end')
            detail.insert('end',f'{title}\n','title')
            detail.insert('end',f'匹配度：{"★"*min(score,5)}{"☆"*max(0,5-score)}  {score}/5\n','score')
            if phase_info: detail.insert('end',f'{phase_info}\n','phase')
            detail.insert('end',f'\n{reason}')
            detail.config(state='disabled')
        ...
        def _on_select(event=None):
            idxs=result_lb.curselection()
            if not idxs or not st.results: return
            item=st.results[idxs[0]]
            _detail_rich(item['ch_title'],item['score'],item['reason'],
                         f'TF-IDF排名: 第{item.get("tfidf_rank","?")}位 → AI精排确认')
            btn_goto.config(command=lambda i=item['ch_idx']:(dlg.destroy(),self.goto_chapter(i)))

        result_lb.bind('<<ListboxSelect>>',_on_select)
        result_lb.bind('<Double-Button-1>',lambda e:btn_goto.invoke() if hasattr(btn_goto,'command') else None)

        def _stage_highlight(stage):
            """高亮当前阶段指示器"""
            lbl_s1.config(fg=_accent if stage==1 else _sel)
            lbl_s2.config(fg=_accent if stage==2 else _sel)
            lbl_s3.config(fg=_accent if stage==3 else _sel)

        def _on_success(results):
            st.running=False; st.results=results
            btn_find.config(text='🔮  重新检索')
            result_lb.delete(0,'end')
            _stage_highlight(3)
            if not results:
                _detail_show('未找到匹配章节，请换一种描述方式。','hint')
                lbl_st2.config(text='未找到'); return
            for r in results:
                stars='★'*min(r['score'],5)
                result_lb.insert('end',f"  第{r['ch_idx']+1}章  {r['ch_title'][:22]}  {stars}")
            lbl_st2.config(text=f'找到 {len(results)} 个匹配')
            result_lb.selection_set(0); _on_select()

        def _on_error(msg):
            st.running=False; btn_find.config(text='🔮  开始两阶段检索')
            _stage_highlight(0)
            if msg.startswith('NO_KEY_'):
                _model=msg.replace('NO_KEY_','')
                _fname='api_key_minimax.txt' if _model=='MiniMax' else 'api_key.txt'
                _detail_show(f'⚠  未找到 {_model} API Key\n请新建 {_fname}','warn')
            else: _detail_show(f'⚠  分析失败\n\n{msg}','warn')
            lbl_st2.config(text='')
        ...
        def _do_find():
            ...
            if st.running: return
            st.running=True; st.results=[]; result_lb.delete(0,'end')
            btn_find.config(text='检索中…')
            btn_goto.config(command=None); lbl_st2.config(text='')
            _tfidf_n=v_tfidf_n.get(); _top_n=v_cnt.get()
            _model_choice=self.var_ai_model.get()

            # 阶段1：本地 TF-IDF 粗筛（主线程，极快）
            _stage_highlight(1)
            _detail_show(f'⏳ 阶段①：本地TF-IDF扫描 {total_ch} 章，提取前 {_tfidf_n} 个候选…','hint')
            dlg.update()

            candidate_idxs=tfidf_rank_chapters(_query,self.chapters,top_n=_tfidf_n)
            lbl_st2.config(text=f'阶段①完成：{len(candidate_idxs)}个候选')

            # 阶段2：AI 精排（异步线程）
            _stage_highlight(2)
            _detail_show(f'⏳ 阶段②：[{_model_choice}] AI精排 {len(candidate_idxs)} 个候选章节…','hint')

            # 构建候选章节TOC（含正文预览，信息更丰富）
            toc_lines=[]
            for i in candidate_idxs:
                ch=self.chapters[i]
                preview=ch['body'][:80].replace('\n',' ').strip()
                toc_lines.append(f"[{i}]《{ch['title']}》：{preview}…")
            toc_text='\n'.join(toc_lines)

            def worker():
                import urllib.request as _ureq, urllib.error as _uerr, json as _uj
                _key=_get_api_key(_model_choice)
                if not _key: dlg.after(0,lambda:_on_error(f'NO_KEY_{_model_choice}')); return
                _url='https://api.minimax.chat/v1/chat/completions' if _model_choice=='MiniMax' else 'https://open.bigmodel.cn/api/paas/v4/chat/completions'
                _model_name='abab6.5s-chat' if _model_choice=='MiniMax' else 'glm-4-flash'
                _prompt=(
                    f"我在读一本小说，对某章节有模糊印象，请从候选章节中找出最匹配的。\n\n"
                    f"【我的描述】\n{_query}\n\n"
                    f"【候选章节（已由TF-IDF预筛选，共{len(candidate_idxs)}个）】\n{toc_text}\n\n"
                    f"请从候选中选出最匹配的 {_top_n} 个，以 JSON 数组返回，每项包含：\n"
                    f"  ch_idx（必须严格使用 [] 中的数字，整数）\n"
                    f"  ch_title（章节标题）\n"
                    f"  score（匹配度1-5分，整数）\n"
                    f"  reason（60字内说明匹配原因）\n\n"
                    f"只返回 JSON，不要其他文字：\n"
                    f'[{{"ch_idx":0,"ch_title":"...","score":4,"reason":"..."}}]'
                )
                _payload=_uj.dumps({'model':_model_name,'max_tokens':600,'temperature':0.2,
                                    'messages':[{'role':'user','content':_prompt}]}).encode('utf-8')
                _req=_ureq.Request(_url,data=_payload,
                                   headers={'Content-Type':'application/json','Authorization':f'Bearer {_key}'},
                                   method='POST')
                try:
                    ...
                    _text=re.sub(r'^```[a-z]*\s*','',_text,flags=re.I)
                    _text=re.sub(r'\s*```$','',_text)
                    _parsed=_uj.loads(_text)
                    results=[]
                    for idx_r,item in enumerate(_parsed[:_top_n]):
                        _ci=int(item.get('ch_idx',0))
                        if 0<=_ci<len(self.chapters):
                            # 记录 TF-IDF 排名位置
                            tfidf_rank=candidate_idxs.index(_ci)+1 if _ci in candidate_idxs else '?'
                            results.append({
                                'rank':len(results)+1,
                                'ch_idx':_ci,
                                'ch_title':self.chapters[_ci]['title'],
                    ...
                    dlg.after(0,lambda r=results:_on_success(r))
                except _uerr.HTTPError as _he:
                    _hbody=_he.read().decode('utf-8',errors='replace')
                    try: _hemsg=_uj.loads(_hbody).get('error',{}).get('message',_hbody)
                    except: _hemsg=_hbody[:150]
                    dlg.after(0,lambda c=str(_he.code),m=_hemsg:_on_error(f'HTTP {c}：{m}'))
                except Exception as _exc: dlg.after(0,lambda m=str(_exc):_on_error(m))

            threading.Thread(target=worker,daemon=True).start()

        btn_find.bind('<Button-1>',lambda e:_do_find())
        inp.bind('<Control-Return>',lambda e:_do_find())

        _detail_show(
            '两阶段检索说明：\n\n'
            '① 本地 TF-IDF 粗筛（零API消耗，毫秒级）\n'
            '   全文扫描，提取关键词最相关的候选章节\n\n'
            '② AI 语义精排（仅分析候选章节，Token消耗少）\n'
            '   深度理解情节语义，输出精准排名\n\n'
            '提示：描述越具体（人物名+情节+场景），准确率越高。','hint')

        _rx=self.root.winfo_x()+self.root.winfo_width()+10; _ry=self.root.winfo_y()
        dlg.geometry(f'520x660+{_rx}+{_ry}')

    # ─────────────────────────────────────────────────
    # ★ 问题1-方案B：角色画像（关键词索引全文扫描）★
    # ─────────────────────────────────────────────────
    def _get_or_create_profile(self, role_name, update_ui_cb):
        safe_title=re.sub(r'[\\/*?:"<>|]','',self.book_title)
        safe_role=re.sub(r'[\\/*?:"<>|]','',role_name)
        ...
        if os.path.exists(profile_path):
            with open(profile_path,'r',encoding='utf-8') as f: return f.read()

        update_ui_cb('系统', f'正在全文扫描【{role_name}】相关段落，构建深度画像…')

        # ★ 方案B核心：全文关键词索引提取，而非只用当前章节 ★
        role_context=extract_role_context(role_name,self.chapters,self.cur_ch,max_chars=8000)
        total_mentions=sum(1 for ch in self.chapters if role_name in ch['body'])

        sys_prompt=(
            f"你是专业的文学角色分析师。根据以下从全书中提取的、包含角色【{role_name}】的段落，"
            f"构建该角色的深度画像。\n"
            f"（角色在全书{len(self.chapters)}章中共出现约{total_mentions}章，已提取最关键段落）\n\n"
            f"画像内容要求（500字以内，直接输出，无需寒暄）：\n"
            f"1. 核心性格特征与人物弧光（成长或堕落轨迹）\n"
            f"2. 标志性说话口癖、语气与惯用语（举原文例句）\n"
            f"3. 对关键人物的态度与人际关系图谱\n"
            f"4. 重要经历、内心创伤或执念"
        )
        messages=[{"role":"system","content":sys_prompt},
                  {"role":"user","content":f"角色相关段落：\n{role_context}"}]
        success,result=_robust_api_call(self.var_ai_model.get(),messages,max_retries=2,timeout=60)
        if success:
            with open(profile_path,'w',encoding='utf-8') as f: f.write(result)
            return result
    ...
    def open_role_chat(self):
        ...
        t=self._cur_theme(); bg,fg,bar,sel=t['bg'],t['fg'],t['bar'],t['sel']
        accent=t.get('accent',sel)
        win=tk.Toplevel(self.root); win.title('与角色跨时空对话')
        win.geometry('460x620'); win.attributes('-topmost',True); win.configure(bg=bg)
        chat_history=[]

        top=tk.Frame(win,bg=bar,height=42); top.pack(fill='x'); top.pack_propagate(False)
        tk.Label(top,text='  💬 对话角色：',font=('',9,'bold'),bg=bar,fg=fg).pack(side='left',padx=4,pady=11)
        role_var=tk.StringVar(value='')
        role_ent=tk.Entry(top,textvariable=role_var,width=14,font=('',10),
                          bg=bg,fg=fg,insertbackground=fg,relief='flat',
                          borderwidth=1,highlightthickness=1,
                          highlightbackground=sel,highlightcolor=accent)
        role_ent.pack(side='left',padx=4,pady=8,ipady=3)

        # 刷新画像按钮
        def refresh_profile():
            role=role_var.get().strip()
            if not role: return
            safe_t=re.sub(r'[\\/*?:"<>|]','',self.book_title)
            safe_r=re.sub(r'[\\/*?:"<>|]','',role)
            pf=os.path.join(PROFILES_DIR,f'{safe_t}_{safe_r}.txt')
            if os.path.exists(pf): os.remove(pf)
            append_msg('系统',f'已清除【{role}】的缓存画像，下次发送将重新分析。','sys')
        ref_lbl=tk.Label(top,text='↺刷新画像',font=('',8),bg=bar,fg=fg,padx=6,cursor='hand2')
        ref_lbl.bind('<Button-1>',lambda e:refresh_profile())
        ref_lbl.pack(side='left',padx=2)
        tk.Label(top,text='×',font=('',13),bg=bar,fg=fg,cursor='hand2',padx=8).pack(side='right')

        chat_frame=tk.Frame(win,bg=bg); chat_frame.pack(fill='both',expand=True,padx=6,pady=6)
        chat_display=scrolledtext.ScrolledText(
            chat_frame,font=(self.font_fam,self.font_size),
            bg=bg,fg=fg,state='disabled',wrap='word',
            borderwidth=0,highlightthickness=0,spacing1=4,spacing3=4)
        chat_display.pack(fill='both',expand=True)
        chat_display.tag_config('name',font=(self.font_fam,self.font_size,'bold'),foreground=accent)
        chat_display.tag_config('user',font=(self.font_fam,self.font_size,'bold'),foreground=sel)
        chat_display.tag_config('sys',font=(self.font_fam,self.font_size-1,'italic'),foreground='#888')

        bot=tk.Frame(win,bg=bar); bot.pack(fill='x',side='bottom')
        msg_ent=tk.Entry(bot,font=(self.font_fam,self.font_size),
                         bg=bg,fg=fg,insertbackground=fg,relief='flat',
                         borderwidth=1,highlightthickness=1,
                         highlightbackground=sel,highlightcolor=accent)
        msg_ent.pack(fill='x',padx=10,pady=8,side='left',expand=True,ipady=5)
        btn_send=tk.Label(bot,text='发送',font=('',10,'bold'),
                          bg=accent,fg='white',padx=14,pady=6,cursor='hand2')
        btn_send.pack(side='right',padx=10,pady=8)

        def append_msg(who,text,tag='name'):
            chat_display.config(state='normal')
            if tag=='sys': chat_display.insert('end',f'[{who}] {text}\n\n','sys')
            elif tag=='user':
                chat_display.insert('end',f'[{who}] ','user')
                chat_display.insert('end',f'{text}\n\n')
            else:
                chat_display.insert('end',f'[{who}] ','name')
                chat_display.insert('end',f'{text}\n\n')
            chat_display.config(state='disabled'); chat_display.see('end')

        def set_ui(enabled=True):
            s='normal' if enabled else 'disabled'
            msg_ent.config(state=s); role_ent.config(state=s)
            btn_send.config(bg=accent if enabled else sel)
            if enabled: msg_ent.focus()

        def do_send(e=None):
            role=role_var.get().strip(); user_text=msg_ent.get().strip()
            if not role or not user_text: return
            if msg_ent['state']=='disabled': return
            msg_ent.delete(0,'end'); set_ui(False)
            append_msg('我',user_text,'user')

            def ai_worker():
                profile=self._get_or_create_profile(role,append_msg)
                if profile.startswith('ERROR:'):
                    win.after(0,lambda:append_msg('系统',f'获取角色画像失败：{profile}','sys'))
                    win.after(0,lambda:set_ui(True)); return
                system_prompt=(
                    f"你正在沉浸式扮演小说角色：【{role}】。\n"
                    f"【你的深度画像（基于全书分析）】:\n{profile}\n\n"
                    f"【扮演规则】:\n"
                    f"1. 完全沉浸角色，绝不承认自己是AI。语言风格、口癖必须与画像一致。\n"
                    f"2. 严禁剧透未来，你只知道画像中涉及的过去与现在。\n"
                    f"3. 保持一问一答的对话感，每次不超过3句话。\n"
                    f"4. 禁止使用侮辱、暴力等违规词汇。"
                )
                messages=[{"role":"system","content":system_prompt}]
                messages.extend(chat_history)
                messages.append({"role":"user","content":user_text})
                success,resp=_robust_api_call(self.var_ai_model.get(),messages)
                win.after(0,lambda:_handle(success,resp,user_text))

            def _handle(success,resp,user_text):
                if success:
                    chat_history.append({"role":"user","content":user_text})
                    chat_history.append({"role":"assistant","content":resp})
                    ...
                    append_msg('系统','NO_KEY'==resp and '请配置 API Key 文件。' or f'对话失败：{resp}','sys')
                set_ui(True)

            threading.Thread(target=ai_worker,daemon=True).start()

        btn_send.bind('<Button-1>',do_send); msg_ent.bind('<Return>',do_send)
        append_msg('系统','已就绪。输入角色名 → 发送消息开始对话。\n首次分析会全文扫描角色相关段落，更准确。','sys')
        role_ent.focus()

    # ─────────────────────────────────────────────────
    # 老板键相关（保持原版逻辑，界面保持高仿Word）
    # ─────────────────────────────────────────────────
    def open_boss_editor(self):
        t=self._cur_theme(); bg,fg,bar=t['bg'],t['fg'],t['bar']
        win=tk.Toplevel(self.root); win.title('编辑伪装文档内容')
        win.geometry('560x480'); win.attributes('-topmost',True); win.configure(bg=bg)
        hdr=tk.Frame(win,bg='#2b579a',height=32); hdr.pack(fill='x'); hdr.pack_propagate(False)
        tk.Label(hdr,text='  📝  编辑 Word 伪装文档内容',
                 font=('',9,'bold'),bg='#2b579a',fg='white').pack(side='left',padx=8,pady=6)
        tk.Label(hdr,text='×',font=('',12),bg='#2b579a',fg='white',cursor='hand2',padx=8).pack(side='right')
        tk.Label(win,text='此内容将显示在 Ctrl+D 触发的 Word 伪装界面中：',
                 font=('',9),bg=bg,fg=fg).pack(anchor='w',padx=12,pady=(10,4))
        ta=tk.Text(win,font=('Microsoft YaHei',10) if sys.platform=='win32' else ('',11),
                   bg='white',fg='#1a1a1a',relief='flat',padx=12,pady=10,wrap='word',
                   borderwidth=1,highlightthickness=1,
                   highlightbackground='#c8c8c8',highlightcolor='#2b579a')
        ta.pack(fill='both',expand=True,padx=12,pady=4)
        if os.path.exists(BOSS_FILE):
            with open(BOSS_FILE,encoding='utf-8') as f: ta.insert('1.0',f.read())
        else: ta.insert('1.0',_DEFAULT_BOSS_TEXT)
        bf=tk.Frame(win,bg=bar); bf.pack(fill='x',padx=12,pady=8)
        tk.Label(bf,text='提示：用 "■ " 开头一级标题，"▸ " 开头二级标题',
                 font=('',8),bg=bar,fg='#888').pack(side='left')
        def save_and_close():
            content=ta.get('1.0','end-1c')
            ...
            if self._is_boss_mode and hasattr(self,'_boss_text_widget'):
                self._refresh_boss_doc(content)
            self._toast('📝 伪装文档已保存'); win.destroy()
        save_lbl=tk.Label(bf,text='保存',font=('',9,'bold'),bg='#2b579a',fg='white',
                          padx=16,pady=4,cursor='hand2')
        save_lbl.bind('<Button-1>',lambda e:save_and_close())
        save_lbl.pack(side='right',padx=4)
        cancel_lbl=tk.Label(bf,text='取消',font=('',9),bg=bar,fg=fg,padx=12,pady=4,cursor='hand2')
    ...
    def _get_boss_content(self):
        if os.path.exists(BOSS_FILE):
            try:
                with open(BOSS_FILE,encoding='utf-8') as f: return f.read()
            except: pass
        return _DEFAULT_BOSS_TEXT

    def _refresh_boss_doc(self,content=None):
        if not hasattr(self,'_boss_text_widget'): return
        if content is None: content=self._get_boss_content()
        tw=self._boss_text_widget
        tw.config(state='normal'); tw.delete('1.0','end')
        for line in content.split('\n'):
            stripped=line.strip()
            if stripped.startswith('■ '): tw.insert('end',stripped[2:]+'\n','h1')
            elif stripped.startswith('▸ '): tw.insert('end',stripped[2:]+'\n','h2')
            elif stripped.startswith('---'): tw.insert('end','\n','normal')
            else: tw.insert('end',line+'\n','normal')
        tw.config(state='disabled'); tw.yview_moveto(0.0)

    def _build_boss_mode_ui(self):
        WORD_BLUE='#2b579a'; RIBBON_BG='#f3f2f1'; DOC_GRAY='#b0b0b0'; BORDER_CLR='#c8c8c8'
        self.boss_frame=tk.Frame(self.root,bg=DOC_GRAY)
        title_bar=tk.Frame(self.boss_frame,bg=WORD_BLUE,height=28)
        title_bar.pack(fill='x'); title_bar.pack_propagate(False)
        tk.Label(title_bar,text='💾  ✔  ↶  ↷',font=('',9),bg=WORD_BLUE,fg='#c7d8f0').pack(side='left',padx=8)
        tk.Label(title_bar,text='项目可行性研究报告.docx - Word',
                 font=('',9,'bold'),bg=WORD_BLUE,fg='white').pack(side='left',expand=True)
        for sym,hov in [('─',WORD_BLUE),('⬜',WORD_BLUE),('✕','#c42b1c')]:
            b=tk.Label(title_bar,text=sym,font=('',10),bg=WORD_BLUE,fg='white',width=4,cursor='hand2')
            b.pack(side='right')
            b.bind('<Enter>',lambda e,w=b,c=hov:w.config(bg=c))
            b.bind('<Leave>',lambda e,w=b:w.config(bg=WORD_BLUE))
        menu_bar=tk.Frame(self.boss_frame,bg=WORD_BLUE,height=24)
        ...
        menus=['文件','开始','插入','设计','布局','引用','邮件','审阅','视图','帮助']
        for i,m in enumerate(menus):
            is_active=(i==1); bg_c='white' if is_active else WORD_BLUE; fg_c=WORD_BLUE if is_active else '#d0dff5'
            lbl=tk.Label(menu_bar,text=m,font=('',9),bg=bg_c,fg=fg_c,padx=10,pady=2,cursor='hand2')
            lbl.pack(side='left')
            if not is_active:
                lbl.bind('<Enter>',lambda e,w=lbl:w.config(bg='#3f6bb5',fg='white'))
                lbl.bind('<Leave>',lambda e,w=lbl:w.config(bg=WORD_BLUE,fg='#d0dff5'))
        ribbon=tk.Frame(self.boss_frame,bg='white',highlightbackground=BORDER_CLR,highlightthickness=1)
        ribbon.pack(fill='x')
        ribbon_inner=tk.Frame(ribbon,bg='white'); ribbon_inner.pack(fill='x',padx=4,pady=0)
        hint=tk.Frame(self.boss_frame,bg=RIBBON_BG,height=14)
        hint.pack(fill='x'); hint.pack_propagate(False)
        tk.Label(hint,text='按 Ctrl+D 恢复阅读',font=('',8),bg=RIBBON_BG,fg='#999').pack(side='right',padx=8)
        ruler=tk.Canvas(self.boss_frame,height=16,bg='#f0eeeb',highlightthickness=0)
        ruler.pack(fill='x')
        self.boss_frame.after(50,lambda:self._draw_ruler(ruler))
        doc_outer=tk.Frame(self.boss_frame,bg=DOC_GRAY); doc_outer.pack(fill='both',expand=True)
        doc_vsb=tk.Scrollbar(doc_outer,orient='vertical',width=14); doc_vsb.pack(side='right',fill='y')
        doc_canvas=tk.Canvas(doc_outer,bg=DOC_GRAY,highlightthickness=0,yscrollcommand=doc_vsb.set)
        doc_canvas.pack(side='left',fill='both',expand=True); doc_vsb.config(command=doc_canvas.yview)
        a4_outer=tk.Frame(doc_canvas,bg=DOC_GRAY)
        doc_canvas.create_window((0,0),window=a4_outer,anchor='nw')
        a4_shadow=tk.Frame(a4_outer,bg='#888',padx=2,pady=2); a4_shadow.pack(pady=20)
        a4=tk.Frame(a4_shadow,bg='white',width=680); a4.pack(); a4.pack_propagate(False)
        doc_font=('Times New Roman',11) if sys.platform!='linux' else ('',11)
        tw=tk.Text(a4,font=doc_font,bg='white',fg='#1a1a1a',relief='flat',padx=80,pady=72,
                   wrap='word',state='disabled',borderwidth=0,highlightthickness=0,
                   spacing1=4,spacing2=2,spacing3=6,cursor='arrow')
        tw.pack(fill='both',expand=True)
        h1_font=('Times New Roman',16,'bold') if sys.platform!='linux' else ('',16,'bold')
        h2_font=('Times New Roman',13,'bold') if sys.platform!='linux' else ('',13,'bold')
        tw.tag_config('h1',font=h1_font,foreground='#2b579a',spacing1=14,spacing3=6)
        tw.tag_config('h2',font=h2_font,foreground='#2b579a',spacing1=10,spacing3=4)
        tw.tag_config('normal',font=doc_font,foreground='#1a1a1a',spacing1=2,spacing3=2)
        self._boss_text_widget=tw; self._boss_doc_canvas=doc_canvas; self._boss_a4_outer=a4_outer
        def _boss_scroll(e):
            delta=-1 if (e.delta>0 or e.num==4) else 1
            doc_canvas.yview_scroll(delta*3,'units')
        doc_canvas.bind('<MouseWheel>',_boss_scroll); doc_canvas.bind('<Button-4>',_boss_scroll)
        doc_canvas.bind('<Button-5>',_boss_scroll); tw.bind('<MouseWheel>',_boss_scroll)
        def _on_configure(e):
            doc_canvas.configure(scrollregion=doc_canvas.bbox('all'))
            cw=doc_canvas.winfo_width(); aw=a4_outer.winfo_reqwidth()
            x=max(0,(cw-aw)//2); doc_canvas.coords(doc_canvas.find_all()[0],x,0)
        a4_outer.bind('<Configure>',_on_configure)
        status=tk.Frame(self.boss_frame,bg='#2b579a',height=22)
        status.pack(fill='x',side='bottom'); status.pack_propagate(False)
        tk.Label(status,text='第 1 页，共 5 页  |  2,847 个字  |  中文(中国)',
                 font=('',8),bg='#2b579a',fg='#c7d8f0').pack(side='left',padx=10)

    def _draw_ruler(self,canvas):
        if canvas is None: return
        try:
            canvas.update_idletasks(); W=canvas.winfo_width(); H=16
            canvas.delete('all'); canvas.create_rectangle(0,0,W,H,fill='#f0eeeb',outline='')
            margin_px=max(0,(W-520)//2)
            canvas.create_rectangle(0,0,margin_px,H,fill='#d0cec9',outline='')
            canvas.create_rectangle(W-margin_px,0,W,H,fill='#d0cec9',outline='')
            for i in range(0,26):
    ...
    def toggle_boss_mode(self,event=None):
        ...
        if self._is_boss_mode:
            ...
            self._real_geometry=self.root.geometry()
            self._real_alpha=self.root.attributes('-alpha')
            for w in (self.topbar,self.setbar,self.sep,self.txt_frame,self.bot_area,self.prog_canvas):
                w.pack_forget()
            if self._tts_playing:
                self.tts.stop(); self._tts_playing=False; self.btn_tts.config(text='🔊')
            self.root.attributes('-alpha',1.0)
            self._refresh_boss_doc()
            sw=self.root.winfo_screenwidth(); sh=self.root.winfo_screenheight()
            w=min(1100,sw-60); h=min(860,sh-60); x=(sw-w)//2; y=max(0,(sh-h)//2-20)
            self.root.geometry(f'{w}x{h}+{x}+{y}')
            self.root.title('项目可行性研究报告.docx - Word')
            self.root.configure(bg='#b0b0b0')
            self.boss_frame.pack(fill='both',expand=True)
            canvas_list=[w for w in self.boss_frame.winfo_children() if isinstance(w,tk.Canvas)]
            if canvas_list: self.root.after(100,lambda:self._draw_ruler(canvas_list[0]))
        else:
            self.boss_frame.pack_forget()
            self.root.geometry(self._real_geometry)
            self.root.title(self._real_title)
            ...
            self.topbar.pack(fill='x')
            self.prog_canvas.pack(fill='x',after=self.topbar)
            if self._settings_open: self.setbar.pack(fill='x',after=self.topbar)
            self.sep.pack(fill='x')
            self.txt_frame.pack(fill='both',expand=True)
            self.bot_area.pack(fill='x',side='bottom')
            self.root.focus_set()
    ...
    # ─────────────────────────────────────────────────
    def _on_close(self):
        self.tts.stop(); self._shelf_update(); self.root.destroy()
