...
# ── 关卡定义（平衡后数值，降低约25-35%难度） ──────────────
LEVELS = [
    {"type":"score",  "steps":22, "target": 800, "star2":1600, "star3":2600},
    {"type":"score",  "steps":22, "target":1000, "star2":1900, "star3":3000},
    {"type":"clear",  "steps":20, "target":14,   "color":0,    "star2":25,  "star3":38},
    {"type":"score",  "steps":20, "target":1400, "star2":2600, "star3":4200},
    {"type":"clear",  "steps":20, "target":18,   "color":1,    "star2":30,  "star3":46},
    {"type":"clear",  "steps":22, "target":22,   "color":-1,   "star2":40,  "star3":62},
    {"type":"score",  "steps":18, "target":1800, "star2":3200, "star3":5200},
    {"type":"clear",  "steps":18, "target":20,   "color":2,    "star2":34,  "star3":52},
    {"type":"score",  "steps":20, "target":2200, "star2":4000, "star3":6400},
    {"type":"clear",  "steps":18, "target":24,   "color":3,    "star2":40,  "star3":60},
    {"type":"score",  "steps":16, "target":2400, "star2":4200, "star3":6800},
    {"type":"clear",  "steps":16, "target":24,   "color":4,    "star2":38,  "star3":58},
    {"type":"score",  "steps":15, "target":2600, "star2":4800, "star3":7800},
    {"type":"clear",  "steps":15, "target":28,   "color":5,    "star2":46,  "star3":68},
    {"type":"score",  "steps":14, "target":3000, "star2":5600, "star3":9000},
]
MAX_LEVELS = len(LEVELS)
...
class AssetLoader:
    ...
    def load(self):
        ...
        for idx, name in enumerate(ANIMAL_NAMES):
            img = self._load(os.path.join(ASSETS_DIR, f"{name}.png"))
            if img:
                self.animal_imgs[idx] = img
            else:
                surf = pygame.Surface((48,48), pygame.SRCALPHA)
                pygame.draw.circle(surf, COLORS[idx]+(180,), (24,24), 22)
                self.animal_imgs[idx] = surf

        for key in ["row","col","bomb","rainbow"]:
            img = self._load(os.path.join(ASSETS_DIR, f"special_{key}.png"))
            if img: self.special_imgs[key] = img

        for key in ["hud_panel","hud_steps_frame","hud_progress_bg",
                    "hud_progress_fill","hud_level_badge"]:
            img = self._load(os.path.join(ASSETS_DIR, f"{key}.png"))
            if img: self.ui_imgs[key] = img

        img = self._load(os.path.join(ASSETS_DIR, "map_bg.png"))
        if img: self.map_imgs["bg"] = img

        img = self._load(os.path.join(ASSETS_DIR, "map_node_locked.png"))
        if img: self.map_imgs["node_locked"] = img

        img = self._load(os.path.join(ASSETS_DIR, "map_path_v.png"))
        if img: self.map_imgs["path_v"] = img

        for i in range(1, MAX_LEVELS+1):
            img = self._load(os.path.join(ASSETS_DIR, f"map_node_{i}.png"))
            if img: self.map_imgs[f"node_{i}"] = img

        for tag in ["on","off"]:
            img = self._load(os.path.join(ASSETS_DIR, f"map_star_{tag}.png"))
            if img: self.map_imgs[f"star_{tag}"] = img
            img2 = self._load(os.path.join(ASSETS_DIR, f"result_star_{tag}.png"))
            if img2: self.result_imgs[f"star_{tag}"] = img2

        for name in ["good","great","amazing","fantastic","unbelievable"]:
            img = self._load(os.path.join(ASSETS_DIR, f"combo_{name}.png"))
            if img: self.combo_imgs[name] = img

        self._loaded = True

    def get_tile(self, bc, sp_key, tw, th):
        key = (bc, sp_key, tw, th)
        ...
        draw_grad_rect(surf, (0,0,tw,th-2), lc, col, radius=10)
        hs = pygame.Surface((max(1,tw-12), max(1,th//3)), pygame.SRCALPHA)
        hs.fill((255,255,255,55)); surf.blit(hs,(6,4))
        if bc in self.animal_imgs:
            scaled = pygame.transform.smoothscale(self.animal_imgs[bc], (tw-4,th-4))
            surf.blit(scaled,(2,2))
        if sp_key and sp_key in self.special_imgs and sp_key!="bomb":
            ov = pygame.transform.smoothscale(self.special_imgs[sp_key],(tw,th))
            surf.blit(ov,(0,0))
...
# ═════════════════════════════════════════ 彩虹粒子 ══
class RainbowParticle:
    COLORS = [(255,80,80),(255,180,30),(255,240,40),
              (60,220,60),(50,160,255),(160,80,255)]
    def __init__(self, cx, cy, index, total):
        self.cx=cx; self.cy=cy; self.index=index; self.total=total
        self.color=self.COLORS[index%len(self.COLORS)]
        self.phase=random.uniform(0,math.tau)
        self.radius=random.uniform(14,20)
        self.size=random.randint(2,4)
        self.speed=random.uniform(1.8,2.8)
    def update(self, dt): self.phase+=self.speed*dt
    def draw(self, surf, tx, ty, alpha=255):
        cx=tx+self.cx; cy=ty+self.cy
        px=int(cx+math.cos(self.phase+self.index*math.tau/self.total)*self.radius)
        py=int(cy+math.sin(self.phase+self.index*math.tau/self.total)*self.radius*0.7)
        ps=pygame.Surface((self.size*2+2,self.size*2+2),pygame.SRCALPHA)
        pygame.draw.circle(ps,self.color+(alpha,),(self.size+1,self.size+1),self.size)
...
# ═════════════════════════════════════ 特殊道具动画 ══
class SpecialAnimManager:
    def __init__(self): self._time={}; self._particles={}
    def reset(self): self._time.clear(); self._particles.clear()
        ...
        for r in range(GRID_SIZE):
            for c in range(GRID_SIZE):
                ...
                if sp in (SPECIAL_ROW,SPECIAL_COL,SPECIAL_BOMB,SPECIAL_RAINBOW):
                    active.add((r,c))
                    self._time[(r,c)]=self._time.get((r,c),0.0)+dt
                    if sp==SPECIAL_RAINBOW and (r,c) not in self._particles:
                        n=6; h=TILE_SIZE//2
                        self._particles[(r,c)]=[RainbowParticle(h,h,i,n) for i in range(n)]
                    if sp==SPECIAL_RAINBOW:
                        for p in self._particles.get((r,c),[]): p.update(dt)
        for k in list(self._time.keys()):
            if k not in active: del self._time[k]
        for k in list(self._particles.keys()):
            if k not in active: del self._particles[k]

    def get_shake(self, r, c, sp):
        t=self._time.get((r,c),0.0)
        if sp==SPECIAL_ROW: return math.sin(t*6.0)*1.8, 0.0
        if sp==SPECIAL_COL: return 0.0, math.sin(t*6.0)*1.8
        return 0.0, 0.0

    def get_bomb_glow(self, r, c):
        t=self._time.get((r,c),0.0)
        return int(60+(math.sin(t*5.0)+1.0)*0.5*120)

    def get_particles(self, r, c): return self._particles.get((r,c),[])


# ═════════════════════════════════════ 连消激励字 ══
COMBO_LABELS = ["good","great","amazing","fantastic","unbelievable"]

class ComboLabel:
    def __init__(self, image, x, y):
        self.image=image; self.x=x; self.y=y
        self.t=0.0; self.dur=1.6; self.rise=28; self.alive=True
    ...
    def draw(self, surf):
        ...
        p=self.t/self.dur
        if p<0.15:      alpha=int(p/0.15*255)
        elif p<0.75:    alpha=255
        else:           alpha=int((1-(p-0.75)/0.25)*255)
        alpha=clamp(alpha,0,255)
        if p<0.12:      sc=0.6+0.4*(p/0.12)
        elif p<0.22:    sc=1.0+0.12*math.sin((p-0.12)/0.10*math.pi)
        else:           sc=1.0
        w=int(self.image.get_width()*sc); h=int(self.image.get_height()*sc)
...
class AnimTile:
    def __init__(self):
        self.ox=self.oy=0.0; self.scale=1.0; self.alpha=255
        self.phase="idle"; self.t=self.dur=0.0
        self.swap_dx=self.swap_dy=0.0; self.fall_from=0.0
        self.pop_particles=[]

    def start_swap(self,dx,dy,dur=0.16):
        self.phase="swap"; self.swap_dx=dx; self.swap_dy=dy; self.t=0.0; self.dur=dur

    def start_fall(self,from_oy,dur=0.22):
        self.phase="fall"; self.fall_from=from_oy; self.oy=from_oy; self.t=0.0; self.dur=dur

    def start_pop(self,color):
        self.phase="pop"; self.t=0.0; self.dur=0.28; self.pop_particles=[]
        for _ in range(10):
            a=random.uniform(0,math.tau); s=random.uniform(2.0,5.0)
            self.pop_particles.append({"vx":math.cos(a)*s,"vy":math.sin(a)*s,
                                       "color":color,"x":0.0,"y":0.0})

    def start_spawn(self,dur=0.18):
        self.phase="spawn"; self.scale=0.0; self.t=0.0; self.dur=dur

    def update(self,dt):
        if self.phase=="idle": return False
        self.t+=dt
        p=min(1.0,self.t/self.dur) if self.dur>0 else 1.0
        eo=lambda t2: 1-(1-t2)**3
        eio=lambda t2: t2*t2*(3-2*t2)
        if self.phase=="swap":
            e=eio(p); self.ox=self.swap_dx*(1-e); self.oy=self.swap_dy*(1-e)
            if p>=1.0: self.ox=self.oy=0; self.phase="idle"; return True
        elif self.phase=="fall":
            self.oy=self.fall_from*(1-eo(p))
            if p>=1.0: self.oy=0; self.phase="idle"; return True
        elif self.phase=="spawn":
            self.scale=eo(p)
            if p>=1.0: self.scale=1.0; self.phase="idle"; return True
        elif self.phase=="pop":
            self.scale=1.0-eo(p); self.alpha=int(255*(1-p))
            for pt in self.pop_particles:
                pt["x"]+=pt["vx"]; pt["y"]+=pt["vy"]; pt["vy"]+=0.25
            if p>=1.0: self.phase="idle"; self.scale=1.0; self.alpha=255; return True
        return False
...
class Match3Pro:
    def __init__(self):
        ...
        self.win_w,self.win_h=BASE_W,BASE_H
        self.screen=pygame.display.set_mode((self.win_w,self.win_h),pygame.RESIZABLE)
        pygame.display.set_caption("萌兽消消乐 ✦")
        self.clock=pygame.time.Clock()
        self.canvas=pygame.Surface((BASE_W,BASE_H))
        ...
        self.current_level=1
        self.anims=[[AnimTile() for _ in range(GRID_SIZE)] for _ in range(GRID_SIZE)]
        self.popups=[]; self.settle_stars=0
        self._btn_retry=self._btn_map_btn=pygame.Rect(0,0,1,1)
        self._reset_level()
    ...
    def _init_fonts(self):
        ...
        self.f_sm  = pygame.font.SysFont(cn or "sans",18)
        self.f_md  = pygame.font.SysFont(cn or "sans",24)
        self.f_lg  = pygame.font.SysFont(cn or "sans",34,bold=True)
        self.f_xl  = pygame.font.SysFont("Arial",50,bold=True)
        self.f_num = pygame.font.SysFont("Arial",46,bold=True)
        self.f_map = pygame.font.SysFont("Arial",18,bold=True)
    ...
    def _reset_level(self):
        ...
        self.anims=[[AnimTile() for _ in range(GRID_SIZE)] for _ in range(GRID_SIZE)]
        self._sp_anim.reset(); self.combo_labels=[]
        self._remove_initial_matches()
        self.selected=None; self.score=0; self.cleared=[0]*6
        self.combo=0; self.busy=False; self.popups=[]
        ld=LEVELS[self.current_level-1]
        self.steps=ld["steps"]; self.obj_type=ld["type"]
        self.obj_target=ld["target"]; self.obj_color=ld.get("color",-1)
        self.star2_thr=ld["star2"]; self.star3_thr=ld["star3"]
    ...
    def _scale(self): return min(self.win_w/BASE_W,self.win_h/BASE_H)

    def _win_to_logic(self,wx,wy):
        s=self._scale(); ox=(self.win_w-BASE_W*s)/2; oy=(self.win_h-BASE_H*s)/2
        return (wx-ox)/s,(wy-oy)/s

    def _grid_offset(self): return (BASE_W-GRID_PX)//2, 192
    ...
    def _all_matches(self):
        ...
        for r in range(GRID_SIZE):
            for c in range(GRID_SIZE-2):
                bc=[self._bc(self.grid[r][c+i]) for i in range(3)]
                if bc[0]>=0 and bc[0]==bc[1]==bc[2]: m.update([(r,c),(r,c+1),(r,c+2)])
        for r in range(GRID_SIZE-2):
            for c in range(GRID_SIZE):
                bc=[self._bc(self.grid[r+i][c]) for i in range(3)]
                if bc[0]>=0 and bc[0]==bc[1]==bc[2]: m.update([(r,c),(r+1,c),(r+2,c)])
        return m
    ...
    def _collect_triggers(self,cells):
        to_del=set(cells); queue=list(cells); vis=set()
        while queue:
            r,c=queue.pop()
            if (r,c) in vis: continue
            vis.add((r,c)); val=self.grid[r][c]; sp=self._sp(val)
            if sp==SPECIAL_ROW:
                for cc in range(GRID_SIZE): to_del.add((r,cc)); queue.append((r,cc))
            elif sp==SPECIAL_COL:
                for rr in range(GRID_SIZE): to_del.add((rr,c)); queue.append((rr,c))
    ...
    def _rainbow_combine(self,r1,c1,r2,c2):
        ...
        if osp in (SPECIAL_ROW,SPECIAL_COL,SPECIAL_BOMB):
            ...
            for row in range(GRID_SIZE):
                for col in range(GRID_SIZE):
                    if self._bc(self.grid[row][col])==obc: self.grid[row][col]=osp+obc
            cells={(row,col) for row in range(GRID_SIZE) for col in range(GRID_SIZE)
                   if self._bc(self.grid[row][col])==obc}
            to_del=self._collect_triggers(cells)
        else:
    ...
    def _trigger_combo_label(self):
        idx=self.combo-2
        if idx<0: return
        idx=min(idx,len(COMBO_LABELS)-1)
        name=COMBO_LABELS[idx]
        if name not in _assets.combo_imgs: return
        img=_assets.combo_imgs[name]
        self.combo_labels.append(ComboLabel(img,BASE_W//2,BASE_H//2-20))
    ...
    def _chain(self,swap_cands=None):
        while True:
            ...
            if new_sp:
                for (r,c),val in new_sp.items():
                    self.grid[r][c]=val; self.anims[r][c].start_spawn(0.35)
                self._frames(30)
            self._fall_refill()
            swap_cands=None
        if self._is_three_star() and self.steps>0:
    ...
    def _is_three_star(self): return self._progress_val()>=self.star3_thr
    ...
    def _add_score(self,gain,cells):
        ...
        if gain>0 and cells:
            ...
            col=(255,235,60) if self.combo<=1 else (255,160,40) if self.combo<=3 else (255,80,80)
            txt=f"+{gain}" if self.combo<=1 else f"x{self.combo}  +{gain}"
            self.popups.append({"text":txt,"x":ox+ca*TILE_SIZE+TILE_SIZE//2,
                                "y":oy+ra*TILE_SIZE,"life":1.2,"color":col})
    ...
    def _bonus_phase(self):
        self.state="SETTLING"; rem=self.steps; self.steps=0
        avail=[(r,c) for r in range(GRID_SIZE) for c in range(GRID_SIZE)
               if self.grid[r][c]>=0 and self._sp(self.grid[r][c])==SPECIAL_NONE]
        random.shuffle(avail); bpos=[]
        for _ in range(rem):
            if not avail:
                avail=[(r,c) for r in range(GRID_SIZE) for c in range(GRID_SIZE)
                       if self.grid[r][c]>=0 and self._sp(self.grid[r][c])==SPECIAL_NONE]
                random.shuffle(avail)
            if not avail: break
            br,bc2=avail.pop(0); bc=self._bc(self.grid[br][bc2])
            if bc<0: bc=0
            spt=random.choice([SPECIAL_ROW,SPECIAL_COL,SPECIAL_BOMB,SPECIAL_BOMB,SPECIAL_RAINBOW])
            self.grid[br][bc2]=SPECIAL_RAINBOW if spt==SPECIAL_RAINBOW else spt+bc
            self.anims[br][bc2].start_spawn(0.3); bpos.append((br,bc2))
            self._frames(14)
        self._frames(45)
        if bpos:
            ad=self._collect_triggers(set(bpos))
    ...
    def _finish(self):
        stars=self._calc_stars(); self.settle_stars=stars
        self._write_save(self.current_level,stars)
        self._frames(50); self.state="RESULT"; self.busy=False
    ...
    def _frames(self,n):
        for _ in range(n):
            ...
            for r in range(GRID_SIZE):
                for c in range(GRID_SIZE): self.anims[r][c].update(dt)
            self._sp_anim.update(dt,self.grid,self._sp)
            self._upd_popups(dt); self._upd_combo_labels(dt)
            self._render()
            for ev in pygame.event.get():
                if ev.type==pygame.QUIT: pygame.quit(); raise SystemExit
                if ev.type==pygame.VIDEORESIZE: self._resize(ev.w,ev.h)
    ...
    def _upd_combo_labels(self,dt):
        for lb in self.combo_labels: lb.update(dt)
        self.combo_labels=[lb for lb in self.combo_labels if lb.alive]
    ...
    def _draw_tile(self,surf,r,c,val,x,y):
        if val<0: return
        an=self.anims[r][c]; bc=self._bc(val); sp=self._sp(val)
        if bc<0: bc=0
        sc=max(0.0,an.scale); tw=int((TILE_SIZE-6)*sc); th=int((TILE_SIZE-6)*sc)
        if tw<2 or th<2: return
        shk_ox,shk_oy=0.0,0.0
        if sp in (SPECIAL_ROW,SPECIAL_COL):
            shk_ox,shk_oy=self._sp_anim.get_shake(r,c,sp)
        ...
        tile=_assets.get_tile(bc,sp_key,tw,th)
        if an.alpha<255: tile=tile.copy(); tile.set_alpha(an.alpha)
        surf.blit(tile,(bx,by))
        if sp==SPECIAL_BOMB:
            ga=self._sp_anim.get_bomb_glow(r,c)
            cx2=bx+tw//2; cy2=by+th//2
            for ring_r in range(tw//2,tw//2+7,2):
                ra2=max(0,ga-(ring_r-tw//2)*20)
                if ra2<=0: break
                rs=pygame.Surface((ring_r*2+2,ring_r*2+2),pygame.SRCALPHA)
                pygame.draw.circle(rs,(255,50,20,ra2),(ring_r+1,ring_r+1),ring_r,2)
                surf.blit(rs,(cx2-ring_r-1,cy2-ring_r-1))
            if "bomb" in _assets.special_imgs:
                ov=pygame.transform.smoothscale(_assets.special_imgs["bomb"],(tw,th))
                surf.blit(ov,(bx,by))
        if sp==SPECIAL_RAINBOW:
        ...
        if an.phase=="pop":
            ...
            for pt in an.pop_particles:
                a2=max(0,int(255*(1-frac)))
                px2=int(x+TILE_SIZE//2+pt["x"]*3); py2=int(y+TILE_SIZE//2+pt["y"]*3)
                ps=pygame.Surface((7,7),pygame.SRCALPHA)
                pygame.draw.circle(ps,pt["color"]+(a2,),(3,3),3)
                surf.blit(ps,(px2-3,py2-3))

    def _draw_progress(self,surf,x,y,w,h=18):
        prog=clamp(self._progress_val()/max(1,self.star3_thr),0,1)
        if "hud_progress_bg" in _assets.ui_imgs:
            bg=pygame.transform.smoothscale(_assets.ui_imgs["hud_progress_bg"],(w,h))
            surf.blit(bg,(x,y))
        else:
            pygame.draw.rect(surf,(28,30,56),(x,y,w,h),border_radius=h//2)
        fw=int(w*prog)
        if fw>4:
            if "hud_progress_fill" in _assets.ui_imgs:
                fill=pygame.transform.smoothscale(_assets.ui_imgs["hud_progress_fill"],(w,h-2))
                clip=pygame.Surface((fw,h-2),pygame.SRCALPHA)
                clip.blit(fill,(0,0))
                surf.blit(clip,(x,y+1))
            else:
                draw_grad_rect(surf,(x,y+1,fw,h-2),(80,230,120),(255,100,50),radius=(h-2)//2)
        stars_got=self._calc_stars()
        for sn in range(1,4):
            thr=[self.obj_target,self.star2_thr,self.star3_thr][sn-1]
            px2=min(x+int(w*thr/max(1,self.star3_thr)),x+w)
            pygame.draw.line(surf,(8,10,24),(px2,y),(px2,y+h),2)
            tag="on" if stars_got>=sn else "off"
            if f"star_{tag}" in _assets.map_imgs:
                si=pygame.transform.smoothscale(_assets.map_imgs[f"star_{tag}"],(18,18))
                surf.blit(si,(px2-9,y-22))
            else:
                col=(255,210,0) if stars_got>=sn else (55,58,90)
                s=self.f_sm.render("★",True,col)
                surf.blit(s,(px2-s.get_width()//2,y-s.get_height()-1))

    def _draw_hud_top(self,cv):
        if "hud_panel" in _assets.ui_imgs:
            panel=pygame.transform.smoothscale(_assets.ui_imgs["hud_panel"],(BASE_W,192))
            cv.blit(panel,(0,0))
        else:
            pygame.draw.rect(cv,(22,24,42),(0,0,BASE_W,185))
            pygame.draw.line(cv,(50,55,90),(0,185),(BASE_W,185),2)

        # 关卡徽章
        if "hud_level_badge" in _assets.ui_imgs:
            badge=pygame.transform.smoothscale(_assets.ui_imgs["hud_level_badge"],(128,28))
            cv.blit(badge,(BASE_W//2-64,8))
        badge_s=self.f_sm.render(f"第 {self.current_level} 关",True,(185,200,245))
        cv.blit(badge_s,(BASE_W//2-badge_s.get_width()//2,14))

        # 步数框
        if "hud_steps_frame" in _assets.ui_imgs:
            sf=pygame.transform.smoothscale(_assets.ui_imgs["hud_steps_frame"],(82,64))
            cv.blit(sf,(BASE_W//2-41,42))
        col_step=(240,248,255) if self.steps>5 else (255,110,70)
        ns=self.f_num.render(str(self.steps),True,col_step)
        cv.blit(ns,(BASE_W//2-ns.get_width()//2,50))
        sl=self.f_sm.render("步",True,(90,100,145))
        cv.blit(sl,(BASE_W//2-41-sl.get_width()-4,60))

        # 目标
        if self.obj_type=="score":
            obj_s=self.f_sm.render(f"目标分数  {self.score:,} / {self.obj_target:,}",True,(175,188,225))
        else:
            cn=COLOR_NAMES[self.obj_color] if self.obj_color>=0 else "任意"
            v=self.cleared[self.obj_color] if self.obj_color>=0 else sum(self.cleared)
            obj_s=self.f_sm.render(f"消除{cn}色  {v} / {self.obj_target}",True,(175,188,225))
            if self.obj_color>=0:
                ic=pygame.Surface((13,13),pygame.SRCALPHA)
                pygame.draw.rect(ic,COLORS[self.obj_color],(0,0,13,13),border_radius=4)
                cv.blit(ic,(BASE_W//2-obj_s.get_width()//2-17,118))
        cv.blit(obj_s,(BASE_W//2-obj_s.get_width()//2,118))

        if self.obj_type=="clear":
            ss=self.f_sm.render(f"分数 {self.score:,}",True,(90,98,140))
            cv.blit(ss,(BASE_W-ss.get_width()-12,118))

        self._draw_progress(cv,28,143,BASE_W-56,18)

    def _draw_playing(self):
        cv=self.canvas; cv.fill((14,14,26))
        self._draw_hud_top(cv)
        ox,oy=self._grid_offset()
        board=pygame.Rect(ox-6,oy-6,GRID_PX+12,GRID_PX+12)
        pygame.draw.rect(cv,(22,24,44),board,border_radius=14)
        pygame.draw.rect(cv,(44,50,82),board,2,border_radius=14)
        for r in range(GRID_SIZE):
            for c in range(GRID_SIZE):
                shade=(28,30,48) if (r+c)%2==0 else (32,35,54)
                pygame.draw.rect(cv,shade,(ox+c*TILE_SIZE,oy+r*TILE_SIZE,TILE_SIZE,TILE_SIZE))
        for r in range(GRID_SIZE):
            for c in range(GRID_SIZE):
                self._draw_tile(cv,r,c,self.grid[r][c],ox+c*TILE_SIZE,oy+r*TILE_SIZE)
        if self.selected:
            sr,sc3=self.selected; sx=ox+sc3*TILE_SIZE; sy=oy+sr*TILE_SIZE
            gl=pygame.Surface((TILE_SIZE+10,TILE_SIZE+10),pygame.SRCALPHA)
            pygame.draw.rect(gl,(255,255,255,30),(0,0,TILE_SIZE+10,TILE_SIZE+10),border_radius=14)
            cv.blit(gl,(sx-5,sy-5))
            pygame.draw.rect(cv,(255,255,255),(sx,sy,TILE_SIZE,TILE_SIZE),3,12)
        for p in self.popups:
            a=min(255,int(255*p["life"]/1.2))
            ...
            ps.set_alpha(a)
            cv.blit(ps,(int(p["x"])-ps.get_width()//2,int(p["y"])))
        for lb in self.combo_labels:
            lb.draw(cv)
        if self.state=="SETTLING":
            bar=pygame.Surface((260,40),pygame.SRCALPHA)
            draw_grad_rect(bar,(0,0,260,40),(44,40,68),(28,24,50),radius=20)
            cv.blit(bar,(BASE_W//2-130,BASE_H-60))
            tt=self.f_sm.render("✨ 步数奖励结算中…",True,(255,215,0))
            cv.blit(tt,(BASE_W//2-tt.get_width()//2,BASE_H-50))

    def _draw_result(self):
        self._draw_playing(); cv=self.canvas
        ov=pygame.Surface((BASE_W,BASE_H),pygame.SRCALPHA)
        ov.fill((4,4,14,215)); cv.blit(ov,(0,0))
        cw,ch=BASE_W-60,330; cx2=30; cy2=185
        card=pygame.Surface((cw,ch),pygame.SRCALPHA)
        draw_grad_rect(card,(0,0,cw,ch),(36,40,68),(22,24,48),radius=24)
        pygame.draw.rect(card,(68,80,128,160),(0,0,cw,ch),2,border_radius=24)
        cv.blit(card,(cx2,cy2))
        passed=self._calc_stars()>0
        ts=self.f_lg.render("关卡完成！" if passed else "本关未通过",True,
                             (80,248,148) if passed else (255,96,70))
        cv.blit(ts,(BASE_W//2-ts.get_width()//2,cy2+14))
        for i in range(3):
            filled=i<self.settle_stars; tag="on" if filled else "off"
            if f"star_{tag}" in _assets.result_imgs:
                si=_assets.result_imgs[f"star_{tag}"]
                sz=72 if filled else 60
                ss=pygame.transform.smoothscale(si,(sz,sz))
                bx=BASE_W//2+(i-1)*78-sz//2; by=cy2+66-(sz-60)//2
                if filled:
                    gl=pygame.Surface((sz+20,sz+20),pygame.SRCALPHA)
                    gl.fill((255,215,0,22)); cv.blit(gl,(bx-10,by-10))
                cv.blit(ss,(bx,by))
            else:
                col=(255,215,0) if filled else (50,52,78)
                ss=self.f_xl.render("★",True,col)
                cv.blit(ss,(BASE_W//2+(i-1)*74-ss.get_width()//2,cy2+70))
        if self.obj_type=="score":
            lines2=[f"分数  {self.score:,}  /  目标 {self.obj_target:,}"]
        else:
            cn=COLOR_NAMES[self.obj_color] if self.obj_color>=0 else "任意"
            v=self.cleared[self.obj_color] if self.obj_color>=0 else sum(self.cleared)
            lines2=[f"消除{cn}色  {v} 个  /  目标 {self.obj_target}",f"总分  {self.score:,}"]
        for i,ln in enumerate(lines2):
            ls=self.f_sm.render(ln,True,(175,188,225))
            cv.blit(ls,(BASE_W//2-ls.get_width()//2,cy2+158+i*26))
        bw2=(cw-30)//2; bh2=50
        self._btn_retry=pygame.Rect(cx2+8,cy2+ch-bh2-14,bw2,bh2)
        self._btn_map_btn=pygame.Rect(cx2+bw2+22,cy2+ch-bh2-14,bw2,bh2)
        for (txt,bx,by,bc) in [("再来一局",cx2+8,cy2+ch-bh2-14,(46,136,72)),
                                ("关卡地图",cx2+bw2+22,cy2+ch-bh2-14,(40,86,148))]:
            lc2=tuple(min(255,bc[i]+32) for i in range(3))
            draw_grad_rect(cv,(bx,by,bw2,bh2),lc2,bc,radius=14)
            pygame.draw.rect(cv,tuple(min(255,bc[i]+60) for i in range(3)),(bx,by,bw2,bh2),1,14)
            bs=self.f_md.render(txt,True,(235,244,255))
            cv.blit(bs,(bx+bw2//2-bs.get_width()//2,by+bh2//2-bs.get_height()//2))

    def _draw_map(self):
        cv=self.canvas
        if "bg" in _assets.map_imgs:
            cv.blit(_assets.map_imgs["bg"],(0,0))
        else:
            cv.fill((10,10,22))

        # 标题面板
        title_bg=pygame.Surface((320,52),pygame.SRCALPHA)
        draw_grad_rect(title_bg,(0,0,320,52),(30,28,60),(18,16,42),radius=26)
        pygame.draw.rect(title_bg,(100,90,160,120),(0,0,320,52),1,border_radius=26)
        cv.blit(title_bg,(BASE_W//2-160,8))
        title=self.f_lg.render("萌 兽 消 消 乐",True,(210,220,255))
        cv.blit(title,(BASE_W//2-title.get_width()//2,16))
        sub=self.f_sm.render("— 选 择 关 卡 —",True,(120,110,180))
        cv.blit(sub,(BASE_W//2-sub.get_width()//2,55))

        pos=self._map_pos()
        unlocked=self.user_data["unlocked"]

        # 路径
        for i in range(1,MAX_LEVELS):
            p1=pos[i-1]; p2=pos[i]; is_un=i<unlocked
            my=(p1[1]+p2[1])//2
            segs=[(p1[0],p1[1],p1[0],my),(p1[0],my,p2[0],my),(p2[0],my,p2[0],p2[1])]
            for x1,y1,x2,y2 in segs:
                if x1==x2 and abs(y2-y1)>4:
                    h=abs(y2-y1)
                    if "path_v" in _assets.map_imgs and is_un:
                        pv=pygame.transform.smoothscale(_assets.map_imgs["path_v"],(8,h))
                        cv.blit(pv,(x1-4,min(y1,y2)))
                    else:
                        col=(60,160,100) if is_un else (46,44,66)
                        pygame.draw.line(cv,col,(x1,min(y1,y2)),(x2,max(y1,y2)),4)
                elif abs(x2-x1)>4:
                    col=(60,160,100) if is_un else (46,44,66)
                    pygame.draw.line(cv,col,(min(x1,x2),y1),(max(x1,x2),y2),4)

        # 节点
        for i in range(MAX_LEVELS):
            lvl=i+1; px,py=pos[i]; is_un=lvl<=unlocked
            stars=self.user_data["stars"].get(str(lvl),0); R=32

            if is_un:
                if stars==3:
                    gl=pygame.Surface((R*2+28,R*2+28),pygame.SRCALPHA)
                    pygame.draw.circle(gl,(255,215,0,28),(R+14,R+14),R+14)
                    cv.blit(gl,(px-R-14,py-R-14))
                nk=f"node_{lvl}"
                if nk in _assets.map_imgs:
                    ni=pygame.transform.smoothscale(_assets.map_imgs[nk],(R*2,R*2))
                    cv.blit(ni,(px-R,py-R))
                else:
                    pygame.draw.circle(cv,(55,135,75),(px,py),R)
                    pygame.draw.circle(cv,(90,195,110),(px,py),R,3)
                # 关卡数字
                ns=self.f_map.render(str(lvl),True,(255,255,255))
                for ox3,oy3 in [(-1,0),(1,0),(0,-1),(0,1)]:
                    nd=self.f_map.render(str(lvl),True,(0,0,0))
                    cv.blit(nd,(px-nd.get_width()//2+ox3,py-nd.get_height()//2+oy3))
                cv.blit(ns,(px-ns.get_width()//2,py-ns.get_height()//2))
                if stars>0:
                    ck=self.f_sm.render("✓",True,(80,255,140))
                    cv.blit(ck,(px-ck.get_width()//2,py-R-12))
            else:
                if "node_locked" in _assets.map_imgs:
                    li=pygame.transform.smoothscale(_assets.map_imgs["node_locked"],(R*2,R*2))
                    cv.blit(li,(px-R,py-R))
                else:
                    pygame.draw.circle(cv,(38,36,58),(px,py),R)
                    pygame.draw.circle(cv,(60,58,88),(px,py),R,2)

            # 星星
            for s in range(3):
                sx=px-16+s*16; sy=py+R+5
                tag="on" if s<stars else "off"
                if f"star_{tag}" in _assets.map_imgs:
                    si=pygame.transform.smoothscale(_assets.map_imgs[f"star_{tag}"],(14,14))
                    cv.blit(si,(sx-7,sy))
                else:
                    col=(255,210,0) if s<stars else (48,46,68)
                    ss=self.f_sm.render("★",True,col)
                    cv.blit(ss,(sx-ss.get_width()//2,sy))

    def _map_pos(self):
        cols=[68,BASE_W//2,BASE_W-68]; row_h=86; sy=90; pos=[]
        for i in range(MAX_LEVELS):
            row=i//3; ci=i%3
            if row%2==1: ci=2-ci
            pos.append((cols[ci],sy+row*row_h))
    ...
    def _map_hit(self,lx,ly):
        for i,(px,py) in enumerate(self._map_pos()):
            if math.hypot(lx-px,ly-py)<=36: return i+1
        return -1
